...
- [x] Write the exact Phase 1 implementation manifest for local room-agent
      ...
      Independent backend and Surface reviews accepted the exact candidates;
      Ocean OS PR #425 and Ocean Surface PR #177 landed on 2026-08-31, with the
      manifest's §15 questions closed by those implementations.
- [x] Implement Phase 1 against that manifest and close its §15 open questions
...
- [x] Implement and independently review the daemon-owned metadata projection, durable ordered replay, authenticated snapshot/live API, and extension admission/binding contract before building the production animated Surface renderer. Implementation (tasks 2–8) is landed and the Task 9 independent review is retained at [`docs/specs/2026-07-20-observatory-gate1-task9-independent-review.md`](docs/specs/2026-07-20-observatory-gate1-task9-independent-review.md); its gating repairs G1–G5 landed as one wave on 2026-09-25 and passed delta review (PR #486; see the review's "Repair wave" section). The renderer gate is open. Of the review's non-gating follow-ups, the size-loop item (the size bound reduced a page-measured excess by raw JSON length) closed in PR #487, which prunes in batches of 64 and re-measures live pages after each commit; the Surface replay scrubber remains.
...
- [ ] Minimizer M2c: after fresh review of the M2b checkpoint and its recorded open decisions
      (argv exposure policy, `default-members`; `SpillingTool::concurrency` forwarding is repaired), thread
      ...
      render an argv call (they read `args.command` today). The concurrency
      forwarding (design line 165) landed as its own checkpoint.
