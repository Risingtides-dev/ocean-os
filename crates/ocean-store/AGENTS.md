...
- **The `room.agent.*` audit rows are records, not prose, and are NOT
  ...
  keeps the evidence.
  The FIFTH read of those rows, `POST /v1/rooms/persistent/{key}/summarize`, is
  now projected too. It shares `read_transcript_page` but builds a model prompt
  rather than a response, so it applies `room_history_text` itself in
  `summary_user_prompt` instead of going through `projected_room_message`;
  `read_transcript_page` stays the one raw paging implementation all three
  consumers share, and the projection sits at each point a prompt or a response
  is SHAPED. That closes the model-laundered route — the audit metadata
  `crates/ocean-daemon/AGENTS.md` says never reaches a model no longer reaches
  the summarizer either, and so cannot ride back out through the summary
  artifact ocean-surface markdown-renders.
  ...
  arm hands anything else through raw on the human, agent, AND summarizer paths
  with no test going red, so a new audit writer lands unprojected and silent —
  add its `type` there in the same commit that adds the row. Second, only the
  BODY is projected: `summary_user_prompt` and every human read still
  interpolate `author_id` verbatim, which is the same unbounded caller-supplied
  identity, and it is bounded at the write side where the id is minted rather
  than in any one of four renderers.
