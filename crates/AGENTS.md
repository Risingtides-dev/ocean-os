...
| `ocean-mcp` | Client connections to external MCP servers and tool adapters; the `ocean-mcp` binary (Ocean as an MCP **server** over stdio — a thin HTTP bridge to the daemon: rooms read/post/join/inspect/resources, agents, sessions, `ocean_prompt`; `setup` installs the `ocean` skill for Claude Code/Codex/Cursor; `doctor`) | Runtime/session/permission authority; subprocess plugins | `ocean-mcp/src/lib.rs`, `provider.rs`, `src/bin/ocean_mcp.rs` | — | `cargo test -p ocean-mcp` |
