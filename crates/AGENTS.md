...
| `ocean-agent-sdk` | Product session/turn/event/surface vocabulary plus strict metadata-only `ocean.extension.service` v1 DTOs and NDJSON bounds | Daemon execution, lifecycle adaptation, transport, and persistence | `ocean-agent-sdk/src/lib.rs`, `ocean-agent-sdk/src/extension_lifecycle.rs` | — | `cargo test -p ocean-agent-sdk && cargo clippy -p ocean-agent-sdk --all-targets -- -D warnings` |
