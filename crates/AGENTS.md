...
| `ocean-daemon` | HTTP/SSE service, execution authority, fail-closed local room operator principal, sole descriptor/reparse-safe read-only four-file extension registry and common exact service-authority derivation, authoritative metadata-only extension lifecycle publication with mismatch-safe project identity and pre-admission terminal guards, checked project snapshot/epoch/reap reconciliation, cancellation-aware concurrent replay/ACK draining with bounded ACK state, boot-local replay/live delivery, strict native stdio supervision, heartbeat/restart/backoff/circuit policy with scope-stable history, bounded redacted stderr, descriptor/file-id-bound executable and assigned roots, minimal environment/secret bindings, generation-safe Unix cleanup with immediate bounded retry and retained process/temp-root authority, unsupported-platform status plus actual-source Windows portability harness, bounded public GitHub projection, room/federation adapters, observatory auth | Provider implementation; client-owned state; extension registry mutations/routes/CLI or durable lifecycle replay; GitHub authentication/writes/aggregation; room durability (owned by ocean-store); Observatory auth logic | `ocean-daemon/src/main.rs`, `extension_registry.rs`, `extension_service.rs`, `extension_lifecycle.rs`, `tests/windows-portability/`, `github.rs`, `room_federation.rs`, `persistent_rooms.rs`, `room_attachments.rs`, `room_operator.rs` | `ocean-daemon/AGENTS.md` | `cargo test -p ocean-daemon` |
