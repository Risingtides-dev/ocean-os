...
- Room memory uses `ocean-memory`'s opaque `SessionMemoryScope::Room` capability, minted only after authoritative room-agent admission. Its partition principal is the exact length-prefixed Room key and its owner is the stable room-wide principal from `room_memory_owner()`; never expose the trusted scope constructor to model/tool arguments or substitute an agent-member owner that silently creates per-agent silos.
