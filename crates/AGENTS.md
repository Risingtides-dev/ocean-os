...
- Room memory uses `ocean-memory`'s opaque `SessionMemoryScope::Room` capability, minted only from an authority-layer private, non-deserializable `RoomMemoryAdmission` implementation after authoritative room-agent admission. Its partition principal is the exact length-prefixed Room key and its owner is the stable room-wide principal from `room_memory_owner()`; never implement the admission trait for request/model/tool DTOs, expose the trusted issuer to tool arguments, or substitute an agent-member owner that silently creates per-agent silos. The store key is `(partition, owner, logical id)`, every legacy `MemoryStore` operation is fixed to the operator partition, and operator ids remain globally unique because legacy get/delete do not carry an owner. `prepare_legacy_rollback` is the explicit downgrade rail: it restores the old operator-only table shape while retaining room rows in a private archive that the next forward migration rehydrates. With the daemon and every database user stopped, operators invoke it through `ocean-memory-rollback --offline-confirm /absolute/path/to/memory.sqlite`; never run that command against a live store.
...
| `ocean-cli` | Thin daemon command/prompt/session client, including the §15 `extension` verbs (reads plus operator-keyed mutations and their 0/3/4 exit contract) | Agent loop and session persistence; any direct extension registry read or write | `ocean-cli/src/main.rs`, `ocean-cli/src/extension.rs` | — | `cargo test -p ocean-cli` |
...
| `ocean-daemon` | HTTP/SSE service, execution authority, fail-closed local room operator principal, sole descriptor/reparse-safe four-file extension registry reader, common exact service-authority derivation, the A3a journaled writer (quarantine, publication marker, recovery, retention, grant diff), its A4 pinned public Git acquisition (public-only DNS, `http.curloptResolve`-pinned stripped-environment `git` process groups, checkout-free tree extraction), and its A3b §15 HTTP adapters (operator-authenticated mutations, common pre-commit/committed envelopes, credential-free list/status reads, startup recovery before readers and services), authoritative metadata-only extension lifecycle publication with mismatch-safe project identity and pre-admission terminal guards, checked project snapshot/epoch/reap and revision-serialized registry reconciliation with a per-package activity ledger, cancellation-aware concurrent replay/ACK draining with bounded ACK state, boot-local replay/live delivery, strict native stdio supervision, heartbeat/restart/backoff/circuit policy with scope-stable history, bounded redacted stderr, descriptor/file-id-bound executable and assigned roots, minimal environment/secret bindings, generation-safe Unix cleanup with immediate bounded retry and retained process/temp-root authority, unsupported-platform status plus actual-source Windows portability harness, bounded public GitHub projection, room/federation adapters, observatory auth | Provider implementation; client-owned state; unpinned, credentialed, or non-HTTPS Git sources or durable lifecycle replay; GitHub authentication/writes/aggregation; room durability (owned by ocean-store); Observatory auth logic | `ocean-daemon/src/main.rs`, `extension_registry.rs`, `extension_registry/transaction.rs`, `extension_registry/transaction/git.rs`, `extension_registry/mutation.rs`, `extension_service.rs`, `extension_lifecycle.rs`, `tests/windows-portability/`, `github.rs`, `room_federation.rs`, `persistent_rooms.rs`, `room_attachments.rs`, `room_operator.rs` | `ocean-daemon/AGENTS.md` | `cargo test -p ocean-daemon` |
| `ocean-extension` | Schema-v1 extension package parsing, filesystem-independent metadata validation, SemVer compatibility, and confined resource validation | Install/trust/enable state; routes; execution | `ocean-extension/src/lib.rs` | `ocean-extension/AGENTS.md` | `cargo test -p ocean-extension` |
...
| `ocean-mcp` | Client connections to external MCP servers and tool adapters; the `ocean-mcp` binary (Ocean as an MCP **server** over stdio — a thin HTTP bridge to the daemon: rooms read/post/join/inspect/resources, agents, sessions, `ocean_prompt`; `setup` installs the `ocean` skill for Claude Code/Codex/Cursor; `doctor`; identity = `--member` > `member.toml` in the Ocean config dir > `OCEAN_MEMBER_ID`, never a made-up default) | Runtime/session/permission authority; subprocess plugins | `ocean-mcp/src/lib.rs`, `provider.rs`, `src/bin/ocean_mcp.rs` | — | `cargo test -p ocean-mcp` |
| `ocean-memory` | Typed provenance-bearing SQLite memory and ingest; capability-safe operator/disabled/opaque Room partitions constrained by partition plus owner; explicit offline legacy rollback preparation | Session transcripts, room admission authority, or shared Bedrock storage | `ocean-memory/src/lib.rs`, `ocean-memory/src/bin/ocean-memory-rollback.rs` | — | `cargo test -p ocean-memory && cargo clippy -p ocean-memory --all-targets -- -D warnings` |
| `ocean-minimizer` | Conservative output minimization for already-tokenized cargo/git/gh/npm/npx/pytest invocations; consumed only by `ocean-runtime`'s default-off M2 gate | Shell parsing/execution; TOML/config; artifacts; runtime/profile wiring | `ocean-minimizer/src/lib.rs::minimize` | `ocean-minimizer/AGENTS.md` | `cargo test -p ocean-minimizer && cargo clippy -p ocean-minimizer --all-targets -- -D warnings` |
...
| `ocean-observatory` | Metadata-only event schema, structural redaction allow-list, cursor semantics, SQLite/WAL persistence with versioned `user_version` migrations to the manifest §4.1 shape, scoped observer auth (HMAC-SHA256), boot-bound mode-0600 token-file distribution, extension admission validation (cycle/depth/cross-authority/idempotency), and in-memory single-use binding tokens | Daemon event adapters or Surface rendering | `ocean-observatory/src/lib.rs`, `ocean-observatory/src/store.rs`, `ocean-observatory/src/migration.rs`, `ocean-observatory/src/auth.rs`, `ocean-observatory/src/admission.rs` | `ocean-observatory/AGENTS.md` | `cargo test -p ocean-observatory` |
...
| `ocean-store` | SQLite durable rooms, rosters, transcripts behind `RoomStore`; local room-agent authority bindings plus immutable approval decisions; inherent access-projection and outbox APIs; restart-safe federation core (credentials, bindings, producer counters, confirmed ingest, trigger-claim journal) | Agent sessions, memory, operator-key custody, HTTP/admission policy, Longhouse titles; federation network client | `ocean-store/src/lib.rs` | `ocean-store/AGENTS.md` | `cargo test -p ocean-store` |
...
- `ocean-minimizer` is a dependency-free M1 library. `ocean-runtime` now depends on it for the M2 command-capture mechanism, but only behind the default-off `SessionContext::command_output_minimization` gate that no harness profile sets, so it is compiled into default builds as a dependency without being live. It stays outside `default-members` until M2c profile wiring makes minimization live (design §3.2); validate it explicitly or through `--workspace`.
