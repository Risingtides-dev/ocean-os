...
| `ocean-daemon` | HTTP/SSE service, execution authority, sole descriptor/reparse-safe read-only four-file extension registry and common exact service-authority derivation, minimum exact-grant native stdio supervision with typed transport-failure causes, descriptor/file-id-bound executable and assigned roots, minimal environment/secret bindings, generation-safe Unix cleanup with retained exceptional-failure authority, unsupported-platform and non-probing runtime status cache plus actual-source Windows portability harness, unwired pure extension lifecycle adaptation, bounded public GitHub projection, room/federation adapters, observatory auth | Provider implementation; client-owned state; extension registry mutations/routes/CLI, lifecycle delivery/replay/live producer wiring, or service health/restart/backoff; GitHub authentication/writes/aggregation; room durability (owned by ocean-store); Observatory auth logic | `ocean-daemon/src/main.rs`, `extension_registry.rs`, `extension_service.rs`, `extension_lifecycle.rs`, `tests/windows-portability/`, `github.rs`, `room_federation.rs`, `persistent_rooms.rs` | `ocean-daemon/AGENTS.md` | `cargo test -p ocean-daemon` |
