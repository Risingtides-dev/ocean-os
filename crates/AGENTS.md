...
| `ocean-daemon` | HTTP/SSE service, per-turn harness-profile composition, execution authority, room access/outbox HTTP projection and retry adapter, outbound Bedrock room client/supervisor | Provider implementation; client-owned state; room access/outbox/federation durability (owned by ocean-store); pending-redemption routes and trigger dispatch (P2-C) | `ocean-daemon/src/main.rs`, `room_federation.rs`, `harness_profile.rs` | `ocean-daemon/AGENTS.md` | `cargo test -p ocean-daemon` |
