...
- The keyless `fake-tool` provider's scripted `write` targets `fake_tool_target_path()`: the fixed `FAKE_TOOL_TARGET_PATH` unless the default-off `test-support` feature is compiled in and `OCEAN_FAKE_TOOL_TARGET_PATH` is set (Stage A O-10). Enable `test-support` only from a dev-dependency (the daemon does); never from a normal dependency, so no production build reads the variable. Release proof: `OCEAN_FAKE_TOOL_TARGET_PATH=/tmp/x cargo test -p ocean-runtime --lib fake_tool` must still target the fixed path.
