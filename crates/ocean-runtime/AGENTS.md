...
- The keyless `fake-tool` provider's scripted `write` targets `fake_tool_target_path()`: the fixed `FAKE_TOOL_TARGET_PATH` unless the default-off `test-support` feature is compiled in and `OCEAN_FAKE_TOOL_TARGET_PATH` is set (Stage A O-10). Enable `test-support` only from a dev-dependency (the daemon does); never from a normal dependency, so no production build reads the variable. Release proof: `cargo test -p ocean-runtime --lib fake_tool` (a `cargo xtask ci` gate; built alone so the feature stays off) sets the variable itself and must still target the fixed path.
...
- Hashline-enabled sessions expose both `edit` and `hashline_edit`; every profile retains `write`. Artifact spill is enabled by the daemon for TUI/ACP/CLI/web and disabled for voice; direct callers default off. Code intelligence (the `lsp` tool) is enabled for TUI/ACP/CLI/web and disabled for voice, whose spoken replies cannot carry definitions/references/diagnostics; direct callers default ON so legacy behavior is unchanged. These are the only effective profile gates currently copied into `SessionContext` from a harness profile. Controlled GPT-5.6 Terra benchmarks showed that hiding `edit` changed model exploration behavior and doubled wall time even when the model still selected `hashline_edit`.
- Minimizer M2 (M2a characterization + M2b runtime mechanism) exists behind `SessionContext::command_output_minimization`, which defaults **off** and which no daemon/agent path sets yet (M2c profile wiring is a separate reviewed checkpoint), so no production turn minimizes anything. When a caller sets it together with `artifacts` and a bound session: the built-in `bash` offers the additive plain-object `command`/`argv` schema (XOR validated before spawn; `argv` executes directly with no shell, sharing cwd/stdin/process-group/timeout/capture/envelope with `command`); only built-in argv-mode `BashTool` (recognized by downcast, never by name) whose `argv[0]` is exactly one of the six bare M1 names is eligible; `SpillingTool::execute_for_run` runs the inner tool once, preserves the artifact-read bypass, spills over-threshold output as before, and otherwise may attach a sealed `ToolExecutionResult` provider projection (exact `\n[exit N]` envelope, no cap markers, >256 bytes saved, strictly smaller final text) pinned to one exact raw artifact via `PinBudget` (32 entries / 1 MiB per active run). The loop emits, checkpoints, and returns raw content; only the per-round provider request clone replaces the result at its exact transcript ordinal, and every lease drops when the run returns. `command` strings, non-`bash` tools, and every uncertainty stay byte-identical. `SpillingTool` forwards `concurrency()` (design §3.6), so a `Shared` read-only tool schedules the same with artifacts on or off; `m2a_characterization` pins the forwarding.
...
- `cargo test -p ocean-runtime --test m2a_characterization --test m2b_command_minimization`
