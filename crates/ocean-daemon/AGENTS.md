...
- `persistent_rooms.rs` owns the private shared room-store handle/lock adapters, durable-room HTTP lifecycle and paging handlers, mention policy evaluation, room-agent auto-convene helper path, and the post-commit transcript/access wake helpers shared with federation. Keep `AppState`, startup store opening, `room_routes()`, call persistence/retries, and LiveKit token authorization in composition. Preserve one concrete `SqliteRoomStore`, poison recovery, no guard across await/event/spawn, exact persisted-row → event → audit-attempt → spawn order, stable room-agent session identity, three-state permission authority, and closed-room audit replay.
- `room_federation.rs` owns the restart-safe outbound Bedrock room client and AppState-owned `FederationSupervisor`: strict origin-only client construction, header-bearer SSE receive/reconnect, roster/presence projection, durable Pending sender scans, status/revoke policy, and post-commit wakes. It never owns SQL, routes, trigger dispatch, or pending-redemption/redeem state (P2-C). Keep exactly one task tree per room, serialize stop/join before the next epoch, select sender Notify + bounded periodic durable scan + cancellation, and never hold `RoomStoreHandle` across network I/O or `.await`. POST 201 never mutates transcript/outbox; ordered SSE is the only ingest/removal rail. Presence follows the authenticated SSE lease, not merely the access-state label. Bearers never enter logs/errors/debug output.
