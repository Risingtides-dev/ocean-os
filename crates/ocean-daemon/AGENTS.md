...
- `GET /v1/rooms/persistent/{key}/events` is the open, non-call persistent-room SSE tail: `Last-Event-ID` (numeric or 400) wins over `after_seq`; frames are `room_message` with the per-room seq id and exact `RoomMessage` JSON. Subscribe before bounded SQLite replay, treat broadcast only as post-commit `(room, seq)` wake hints, and recover gaps or `Lagged` by paging from `last_sent_seq` in order with deduplication. Unknown/closed rooms return 404, `call:` rooms return the typed unsupported rejection, and the stream uses the shared 3-second keepalive plus shutdown wrapper. Every non-call production transcript writer must publish only after its allocating transaction commits.
