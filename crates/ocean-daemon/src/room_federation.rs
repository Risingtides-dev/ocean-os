...
/// - `flush_write` is PER-FILE — a 3,000-file flush must not become 3,000
///   transcript rows; the single `flushed` row carries the outcome.
/// - `file_written` / `file_deleted` / `mkdir` are per-operation editor
///   traffic, the workspace equivalent of keystrokes.
...
mod tests {
    ...
    #[test]
    fn workspace_marker_allowlist_admits_outcomes_and_rejects_noise() {
        for signal in [
            "room.workspace.provisioned",
            "room.workspace.destroyed",
            "room.workspace.repo_cloned",
