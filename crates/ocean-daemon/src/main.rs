...
//
// A scrapable observability surface (`GET /metrics`, Prometheus text format)
// built entirely from hand-rolled relaxed atomics — the same lock-free pattern
// as `AppState::persist_failures`, so recording a turn never contends a lock on
// the hot path. No metrics crate is pulled in (none is a workspace dep, and the
// daemon's whole instrumentation budget for this is "a few `fetch_add`s"). All
// state and the renderer live in this one block so concurrent edits to the rest
// of `main.rs` don't collide with it.
...
impl TurnMetrics {
    ...
    fn record_turn(&self, wall_ms: u64, ok: bool) {
...
impl InFlightGuard {
    fn enter(metrics: Arc<TurnMetrics>) -> Self {
        metrics
            .in_flight
...
async fn agent_turn(
    ...
    // `tools` + `tools/` filenames) so the turn can narrow to it below.
    let (guided_prompt, agent_tool_allowlist): (String, Option<Vec<String>>) =
    ...
    // turn's toolset (fail-safe to the full set if it matches nothing). No-op
    // for every non-agent turn (`agent: None`).
...
mod tests {
    ...
    fn labelled_value(body: &str, name_with_labels: &str) -> Option<u64> {
        body.lines().find_map(|line| {
