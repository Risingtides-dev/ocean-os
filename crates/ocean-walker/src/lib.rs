...
//!
//! # Overview
//! `ocean-walker` owns a standalone native directory-read fast path for future
//! filesystem consumers. This M1 checkpoint is not wired to runtime grep, glob,
//! AST scans, shell builtins, or any production capability. The crate exposes
//! plain Rust types, visitor interfaces, cache policy, and a caller-supplied
//! heartbeat without importing N-API dependencies.
//!
//! # Security boundary
//! [`FollowLinks`] and [`WalkOptions::same_file_system`] are snapshot traversal
//! selection policies, not sandbox or root-confinement controls. M1 does not
//! authorize adversarial roots. Live runtime adoption requires separate
//! point-of-use descriptor/handle-relative confinement against rename,
//! symlink/reparse-point, and cached-candidate swaps on every supported OS.
//! Cancellation is checked at request entry and periodically during traversal,
//! but can be deferred while one native directory syscall yields a giant batch.
...
pub use cache::{
    ...
    invalidate_path, invalidate_path_string, max_cache_entries, normalize_display_relative_path,
...
///
/// This selects links visible in one traversal snapshot. It is not a sandbox
/// or a guarantee that operational paths remain below the initial root.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
...
impl WalkFilter {
    ...
    fn accepts_collected(&self, entry: &CollectedEntry) -> bool {
        ...
        accepts_kind && self.accepts_path(&entry.display_path)
    }
    ...
    fn stream_decision(&self, meta: &EntryMeta<'_>) -> WalkDecision {
        ...
        if self.accepts_path(meta.display_relative_path) {
...
pub struct EntryMeta<'a> {
    ...
    /// Lossy normalized UTF-8 relative projection used for display and
    /// filtering only.
    pub display_relative_path: &'a str,
...
impl<'a> EntryMeta<'a> {
    ...
    pub fn from_collected(root: &'a Path, entry: &'a CollectedEntry) -> Self {
        Self {
            ...
            display_relative_path: &entry.display_path,
    ...
    pub const fn from_entry(root: &'a Path, entry: &Entry<'a>) -> Self {
        Self {
            ...
            display_relative_path: entry.display_relative,
            file_type: entry.file_type,
            mtime: entry.mtime,
            size: entry.size,
            depth: entry.depth,
...
pub struct FileCandidate {
    /// Absolute filesystem path to the regular file, built from the exact native
    /// relative path rather than the display projection.
    pub path: PathBuf,
    /// Lossy normalized UTF-8 relative path for display and filtering only.
...
impl FileCandidate {
    fn from_entry(root: &Path, entry: CollectedEntry) -> Self {
        Self {
            ...
            display_relative: entry.display_path,
            mtime: entry.mtime,
            size: entry.size,
        }
    }
...
pub struct WalkStats {
    /// Age of a cached entry in milliseconds. A cache hit can be younger than
    /// one millisecond and report zero; use [`WalkOutcome::backend`] for
    /// provenance.
...
pub struct WalkOptions {
    ...
    /// Select entries on the root filesystem when supported by the platform.
    /// This snapshot filter is not a root-confinement or sandbox boundary.
...
impl WalkRequest {
        ...
        E: fmt::Display,
    {
        ...
        if scan.entries.is_empty() && self.should_recheck_empty(scan.backend, scan.cache_age_ms) {
            options.cache = false;
    ...
    fn rank_entries(entries: &mut [CollectedEntry], rank: WalkRank) {
        match rank {
            WalkRank::PathAsc => entries.sort_by(|left, right| {
                left.display_path
                    .cmp(&right.display_path)
                    .then_with(|| left.native_relative_path.cmp(&right.native_relative_path))
            }),
    ...
    fn should_recheck_empty(&self, backend: WalkBackend, cache_age_ms: u64) -> bool {
        if backend == WalkBackend::Fresh {
            return false;
        }
    ...
    S: Fn(&FileCandidate) -> std::result::Result<ParallelWalkControl, E> + Sync,
{
    ...
    fn visit_pre_decided(
        ...
        let candidate = FileCandidate {
            ...
            display_relative: entry.display_relative.to_string(),
            mtime: entry.mtime,
            size: entry.size,
        };
            ...
            match decision {
                WalkDecision::Include => {
                    if entry.file_type == FileType::File && next_depth >= context.options.min_depth
                    {
                        let candidate = FileCandidate {
                            path: absolute.clone(),
                            display_relative: relative.clone(),
                            mtime: entry.mtime,
                            size: entry.size,
                        };
...
pub struct CollectedEntry {
    /// Exact native relative filesystem path from the traversal root.
    pub native_relative_path: PathBuf,
    /// Lossy normalized UTF-8 projection for display, filtering, and ordering
    /// only. Never use this field to open a filesystem object.
...
impl CollectedEntry {
    ...
    pub fn depth(&self) -> usize {
        self.native_relative_path.components().count()
    }
...
pub fn sort_collected_depth_first(entries: &mut [CollectedEntry]) {
    entries.sort_unstable_by(|left, right| {
        compare_depth_first_paths(&left.display_path, &right.display_path)
            .then_with(|| left.native_relative_path.cmp(&right.native_relative_path))
    });
}
...
pub struct CollectedEntries {
    ...
    pub entries: Vec<CollectedEntry>,
    /// Age of a cached entry in milliseconds. Cached sub-millisecond hits can
    /// report zero, so use [`CollectedEntries::backend`] for provenance.
    pub cache_age_ms: u64,
    /// Explicit fresh-scan versus cache-hit provenance.
    pub backend: WalkBackend,
}
...
struct CollectVisitor<'root, E> {
    root: &'root Path,
    entries: Vec<CollectedEntry>,
    _error: std::marker::PhantomData<fn() -> E>,
}

impl<'root, E> CollectVisitor<'root, E> {
    const fn new(root: &'root Path) -> Self {
        Self {
            root,
            entries: Vec::new(),
...
impl<E> EntryVisitor for CollectVisitor<'_, E> {
    ...
    fn visit(&mut self, entry: Entry<'_>) -> std::result::Result<WalkControl, Self::Error> {
        let native_relative_path = entry
            .path
        ...
        self.entries.push(CollectedEntry {
            ...
            display_path: entry.display_relative.to_string(),
            file_type: entry.file_type,
            mtime: entry.mtime,
            size: entry.size,
        });
...
where
    H: FnMut() -> std::result::Result<(), E>,
{
    let mut collector = CollectVisitor::new(root);
    ...
    if options.contents_first {
        ...
        collector.entries.sort_unstable_by(|a, b| {
            a.display_path
                .cmp(&b.display_path)
                .then_with(|| a.native_relative_path.cmp(&b.native_relative_path))
        });
    }
    Ok(CollectedEntries {
...
impl<H> WalkContext<'_, H> {
        ...
        H: FnMut() -> std::result::Result<(), V::Error>,
    {
        ...
        if self.options.follow_links == FollowLinks::Always {
            if let Ok(id) = directory_identity(root) {
    ...
    {
        if self.options.follow_links == FollowLinks::Always {
            if let Ok(identity) = directory_identity(&self.absolute_path) {
        ...
        H: FnMut() -> std::result::Result<(), V::Error>,
    {
        ...
        let decision = {
            let meta = EntryMeta {
                ...
                display_relative_path: &self.relative_path,
                file_type,
                mtime,
                size,
                depth: next_depth,
        ...
        if !self.options.contents_first && decision.emit && next_depth >= self.options.min_depth {
            match visitor
                .visit_pre_decided(Entry {
                    ...
                    display_relative: &self.relative_path,
                    name,
                    file_type,
                    mtime,
                    size,
        ...
        if state.chain_has_matchers {
            ...
            while let Some(frame) = current {
                if ignore_match.is_none() {
                    if let Some(matcher) = &frame.ignore_matcher {
                        ignore_match = matcher.matched(path, is_dir);
                    }
                }
                if gitignore_match.is_none() {
                    if let Some(matcher) = &frame.gitignore_matcher {
                        gitignore_match = matcher.matched(path, is_dir);
                    }
                }
                if any_git && !saw_git && git_exclude_match.is_none() {
                    if let Some(matcher) = &frame.git_exclude_matcher {
...
mod platform {
    ...
    use super::{
        mtime_millis, FileType, RawDirEntry, ReadDirControl, ReadDirError, WalkDetail, WalkError,
    };
...
mod tests {
    ...
    fn temp_tree(name: &str) -> TempTree {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root = std::env::temp_dir().join(format!("ocean-walker-{name}-{unique}"));
    ...
    fn walk_request_stream_fallback_preserves_predicate_skip_descend() {
        ...
        impl EntryVisitor for RecordingVisitor {
            ...
            fn visit(&mut self, entry: Entry<'_>) -> std::result::Result<WalkControl, Self::Error> {
                self.paths.push(entry.display_relative.to_string());
                Ok(WalkControl::Continue)
            }
        }
        ...
        let status = WalkRequest::from_options(tree.path(), test_options())
            ...
            .stream_with_predicate(&mut visitor, |meta: &EntryMeta<'_>| {
                if meta.display_relative_path == "skip" {
                    WalkDecision::SkipDescend
                } else {
                    WalkDecision::Include
                }
    ...
    impl EntryVisitor for PruneVisitor {
        ...
        fn visit(&mut self, entry: Entry<'_>) -> std::result::Result<WalkControl, Self::Error> {
            self.seen.push(entry.display_relative.to_string());
            if entry.display_relative == "skip" {
