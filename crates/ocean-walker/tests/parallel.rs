...
#[cfg(unix)]
use ocean_walker::FollowLinks;
use ocean_walker::{
    walk_entries, CompiledWalkGlob, Entry, EntryVisitor, ParallelWalkControl, WalkControl,
    WalkError, WalkFilter, WalkOptions, WalkOrder, WalkRequest, WalkStatus,
};
...
impl TempTree {
    fn new(name: &str) -> Self {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root = std::env::temp_dir().join(format!("ocean-walker-parallel-{name}-{unique}"));
...
fn sorted_serial_candidates(request: &WalkRequest) -> Vec<String> {
    let mut paths = request
        ...
        .into_iter()
        .map(|candidate| candidate.display_relative)
        .collect::<Vec<_>>();
    paths.sort_unstable();
    paths
}
        ...
        .for_each_file_candidate_parallel(
            {
                ...
                move |candidate| {
                    paths
                        ...
                        .push(candidate.display_relative.clone());
...
#[test]
fn parallel_walk_checks_heartbeat_before_invalid_depth_early_return() {
    let tree = TempTree::new("heartbeat-invalid-depth");
    ...
    match result {
        Err(WalkError::Interrupted("heartbeat failed")) => {}
        other => {
            panic!("invalid-depth walks should still return the heartbeat error, got {other:?}")
        }
    }
...
#[test]
fn collected_entries_preserve_distinct_invalid_byte_paths_despite_lossy_collision() {
    let tree = TempTree::new("invalid-byte-identity");
    let first_name = OsString::from_vec(b"collision-\xFE.txt".to_vec());
    let second_name = OsString::from_vec(b"collision-\xFF.txt".to_vec());
    let first_path = tree.path().join(&first_name);
    let second_path = tree.path().join(&second_name);
    fs::write(&first_path, b"first").expect("first invalid-byte file should be written");
    fs::write(&second_path, b"second").expect("second invalid-byte file should be written");

    let entries = WalkRequest::new(tree.path())
        .filter(WalkFilter::files_only())
        .collect()
        .expect("invalid-byte collection should succeed")
        .entries;
    assert_eq!(entries.len(), 2);
    assert_eq!(entries[0].display_path, entries[1].display_path);
    assert_ne!(
        entries[0].native_relative_path,
        entries[1].native_relative_path
    );

    let mut opened = entries
        .iter()
        .map(|entry| {
            let absolute = entry.absolute_path(tree.path());
            assert!(absolute == first_path || absolute == second_path);
            fs::read(absolute).expect("exact native collected path should be openable")
        })
        .collect::<Vec<_>>();
    opened.sort();
    assert_eq!(opened, vec![b"first".to_vec(), b"second".to_vec()]);
}
