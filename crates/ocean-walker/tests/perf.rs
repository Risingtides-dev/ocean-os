//! Ignored deterministic timing harness for ocean-walker.
//!
//! Run with:
//! cargo test --release -p ocean-walker --test perf -- --ignored --nocapture
//! --test-threads=1
...
use ocean_walker::{WalkDetail, WalkOrder, WalkRequest};
...
#[test]
#[ignore = "run with: cargo test --release -p ocean-walker --test perf -- --ignored --nocapture \
            --test-threads=1"]
...
fn build_synthetic_tree() -> PathBuf {
    let root = unique_temp_root("ocean-walker-perf");
...
fn synthetic_content(file_index: usize) -> String {
    ...
    let filler = format!("line {file_index:05} deterministic ocean walker payload text\n");
