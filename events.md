...
time:      [14:05] [08-09-26]
agent:     [claude] [fable-5.1]
worktree:  cc/rooms-phase2-2c
type:      [feature-request]
area:      [backend]

Stage 2b merged (#455) and deployed from the clean origin/main worktree (daemon rev 3996c68c); proven live on the campaigns room: PUT profile with three credential slots resolved CODEX and CLAUDE against the daemon's own auth.json oauth blocks, GH_TOKEN reported resolver_not_open for keychain, and a value-level leak check found no auth.json string in the inspect or profile bodies. Stage 2c implemented under the program: ocean-store room_resources.rs (room_resource_grants with a live-root partial unique index, room_resource_decisions, the ordered access ladder, effective_status where expiry reads revoked, generation-bumping suspend/resume/revoke, revoked terminal, expired-root retirement in-transaction, one room-wide consumed_decision_on across all three ledgers; 7 tests) and ocean-daemon room_resources.rs (six operator-gated/credential-free routes, every §4 dangerous root refused by name via canonicalize-once + O_DIRECTORY|O_NOFOLLOW handle, confine() on the canonical result with symlink-escape tests, a rootless §7.4 projection shared by routes and inspect, and resolve_turn_cwd implementing §5 as ruled in §11.4 for BOTH the convene path and inspect's per-agent execution). Profile PUT now refuses dangling resource refs with resource_not_found instead of phase_not_open. Route baseline 127 -> 133, guide entries, manifest §7/§9 updated. Gates: store 229 tests, room daemon modules green, clippy -D warnings clean after gating the 2d-only confine helper.
_________________________________________________________________________________ 14:05 cc/rooms-phase2-2c
time:      [15:10] [08-09-26]
agent:     [claude] [fable-5.1]
worktree:  cc/rooms-phase2-2c
type:      [feature-request]
...
Stage 2c merged (#456). Stage 2d implemented under the program: ocean-agent room_resources.rs adds the admitted room_list / room_read tools in the room_history shape (opaque handle from admission evidence + a daemon-owned RoomResourceAuthority consulted with the fixed scope on EVERY call; model args are a resource_id and a relative path only), confine() on the canonical result (moved here from the daemon), Decision 8 budgets (2,000 non-recursive entries, 8 MiB per file, 64 KiB/512 KiB chunks with next_offset, 30 s deadline), NUL-detection for binaries, and one audit fact per call with a path digest; PromptControl gains with_room_resources and apply_admitted_room_tools strips the two reserved names; system prompt gains a contributed-folders section. ocean-store adds room_resource_audit. ocean-daemon adds DurableRoomResourceAuthority (re-checks room_agent_generation_is_active + grant status/agent/mode/root liveness per call, records audit with actor agent|operator_preview), admitted_resource_catalog, convene wiring (handle attached only when a grant admits the agent; refusal code room_resources_unavailable), and operator PREVIEW routes POST .../resources/{id}/list|read that run the same tools under the agent's current binding generation. Route baseline 133 -> 135; guide; manifest §7/§9 and the tool-presence deviation re-scoped to Phase 3. Gates: agent 245, store 230, room daemon modules 152 tests; clippy across the three crates after two nits; workspace check.
_________________________________________________________________________________ 15:10 cc/rooms-phase2-2c
...
Addressed PR #450 findings 3960890580 and 3960890591. HTTP Room close now linearizes with final request admission and cancels exact-Room active turns while preserving unrelated sessions. Closed Rooms fail current generation checks even with retained audit bindings. Profile/resource, cursor, credential, binding, outbox and legacy content writers guard openness within their write transaction or atomic statement, including retries after retention. Content-free operation audit and secret revocation remain allowed. Daemon 937/937 and store 254/254 tests pass, including close cancellation, retained binding denial, closed replay and two-connection projection/outbox preservation. Formatting, diff hygiene and docs-check pass. Updated nearest daemon/store contracts and removed stale Phase 2 placeholder descriptions; moved source ownership notes out of the store Child devlog Index. Parent ownership/indexes unchanged. Full workspace, remote CI and fresh exact-head review follow; no production install claimed.
_________________________________________________________________________________ 14:31 codex/rooms-release-20260908
time:      [16:05] [08-09-26]
agent:     [claude] [fable-5.1]
worktree:  cc/rooms-phase2-close
type:      [goal]
area:      [backend]
...
time:      [15:51] [09-09-26]
agent:     [claude] [fable-5.1]
worktree:  cc/team-onboarding-kit
type:      [feature-request]
area:      [docs]

Teammate onboarding kit so a Rising Tides coworker on an Apple-silicon Mac reaches Ocean rooms in about fifteen minutes without the operator on the call. docs/TEAM_ONBOARDING.md is the runbook: what a member gets, the four operator prerequisites (tailnet tail168656.ts.net, KINGMAKER-SYSTEMS org, Cloudflare, a Bedrock invite link), steps (a)–(h) from `tailscale up` through `ocean-mcp doctor` and an `ocean_room_read` from Claude Code, federation facts, a dated team-status table (Eric/Jake/Jay on tailnet+org, Johnny not in the org yet, only Eric running a daemon, nobody on ocean-mcp or in the room), and troubleshooting (/health not /v1/health, the launcher's `federation=on (file)` line and its refusal codes, recovering vs live, the repo-cwd guard, redeem error codes). ops/onboard-teammate.sh is the idempotent machine side: macOS arm64 check, bun+gh, read:packages fail-closed, ~/.npmrc scope lines without duplication, `bun add -g @risingtides-dev/ocean`, a 0600 federation.env in a 0700 dir that never replaces a real owner credential without --force, the same dev.risingtides.ocean-daemon LaunchAgent shape as ops/install-ocean-daemon.sh (package daemon copied to ~/.local/libexec/ocean-daemon/current, launcher copy, $HOME cwd, OCEAN_YOLO=1, OCEAN_MODEL pinned, guarded bootout/bootstrap/kickstart, /health wait), `ocean-mcp setup`, `claude mcp add`, and a final checklist with the redeem placeholder; `--dry-run` prints instead of mutating. Three facts from source shaped it and are recorded in the doc rather than papered over: FederationConfig::resolve and the launcher both refuse a URL-only federation.env although the owner token is only used for owner bootstrap, so the script writes a fixed non-credential marker token line; origin has no v* tag and no release, so the package is not yet installable; and release.yml on main stages no ocean-mcp binary although package.json and test-package.sh require it. Verified: bash -n and shellcheck -S style clean; harnesses proved federation.env idempotency/protection/--force, npmrc no-duplication and token refresh, the rendered plist lints with no federation key, and deploy/ocean-daemon.sh accepts the written file and reports federation=on (file); docs-check PASS; check-ledger clean. No daemon or launchctl was touched on this machine.
_________________________________________________________________________________ 15:51 cc/team-onboarding-kit
time:      [16:40] [09-09-26]
agent:     [claude] [fable-5.1]
worktree:  cc/federation-member-node
type:      [bug-report]
area:      [backend]

The teammate onboarding kit (PR #460) found that a coworker's daemon configured with only OCEAN_FEDERATION_URL was refused: FederationConfig::resolve and the launcher's federation_load both demanded an owner token even though the token is only ever sent on owner bootstrap, so invite redemption on a member node answered 503 federation_unavailable while docs/OPERATIONS.md claimed it worked. Made the owner token optional end to end: the daemon resolver treats an absent or explicitly empty token as a MEMBER node (client on, owner_token None; a token without an origin is still InvalidConfig; a malformed present token is still refused), parse_federation_config returns Option for the token, the launcher exports the origin alone and logs federation=on (file, member), and ops/set-ocean-federation.sh gains --member (origin only, no bearer source). OPERATIONS.md gains "Join as a member node". Tests: url_only_private_config_file_is_a_member_node plus the process-pair test now pins origin-without-token as member, empty token as member, and token-without-origin as invalid; launcher harness verified both file shapes. With that in place, ops/onboard-teammate.sh and docs/TEAM_ONBOARDING.md (merged in #460) drop the fake owner-token marker line and write the origin-only member file the daemon now accepts, tagged with a comment line for re-run detection.
_________________________________________________________________________________ 16:40 cc/federation-member-node
time:      [18:20] [09-09-26]
agent:     [claude] [fable-5.1]
worktree:  cc/rooms-s0-participant-retirement
type:      [feature-request]
area:      [backend]

Rooms S0, the daemon half of the design direction landed as ocean-surface PR #220 (a three-lens judged design pass). ocean-store gains room_retirement.rs: room_participant_aliases + room_retirement_decisions (joined into the room-wide consumed_decision_on namespace) and retire_participant, one IMMEDIATE transaction that moves the Local owner role and every room_agent_owners row from a placeholder human to a real member, drops the placeholder's roster row, records the alias, and appends one room.participant.retired System row (4 tests: owner/agents/roster/alias/audit; second retirement + chains; replay idempotent, changed content and cross-ledger reuse refused; refusals write nothing). ocean-daemon gains room_retirement.rs with POST /v1/rooms/persistent/{key}/participants/{id}/retire (operator lane, replay-safe); only surface-operator and ^web-[0-9a-f]{16}$ are retirable so the route can never fold a real person or an agent into someone; prove_owner_and_target resolves a binding's frozen owner_member_id through the alias chain, so the successor can suspend/reauthorize what the placeholder authorized without a ledger rewrite; inspect, room detail, and snapshot carry aliases. room_history_text now appends the roster-public agent id after the four bracket labels, renders refused admissions as [room agent admission refused: {code}] {agent}, and renders retirements as Participant retired: a -> b (the leak tests now forbid only operator principal, decision id, and raw type strings). ocean-mcp resolves identity as --member > member.toml in the Ocean config dir > OCEAN_MEMBER_ID and refuses join/post without one (no more $USER/operator default). Route baseline 137 -> 138; operator guide gains the route and an "Operator lane" section documenting operator.key and OCEAN_OPERATOR_ALLOWED_ORIGINS. Spec: docs/specs/2026-09-09-ocean-rooms-participant-retirement.md.
_________________________________________________________________________________ 18:20 cc/rooms-s0-participant-retirement
time:      [20:12] [09-09-26]
agent:     [claude] [fable-5.1]
worktree:  cc/rooms-s0-campaigns-migration
type:      [workflow]
area:      [infra]

S0 merged (#462) and deployed from the clean origin/main worktree (daemon rev f458f2ea); ~/.local/bin/ocean-mcp rebuilt from the same tip and ~/.config/ocean-rs/member.toml written with member_id = "smaths", so the bridge reports "member id: smaths". Ran the campaigns migration the spec calls for: POST /v1/rooms/persistent/campaigns/participants/surface-operator/retire {successor_id: smaths} under decision 878907a3-1874-467d-8a55-f5daab6437e3 (owner_moved true, agents_moved 2) and .../web-18c11f5d551e63f8/retire under decision 87b3a3c9-c46f-4bd9-b924-334b740eed0b (owner_moved false, agents_moved 0). After: inspect shows owner smaths (eligible), humans [smaths], aliases surface-operator->smaths and web-18c11f5d551e63f8->smaths; the agents route shows room-builder's binding still frozen to owner surface-operator with owner_eligible true through the alias; the transcript tail reads "[room agent output audit] room-builder", "Participant retired: surface-operator -> smaths", "Participant retired: web-18c11f5d551e63f8 -> smaths". One person, one id, on the live room.
_________________________________________________________________________________ 20:12 cc/rooms-s0-campaigns-migration
time:      [20:40] [09-09-26]
agent:     [claude] [fable-5.1]
worktree:  cc/rt-hq-first-ops-agent
type:      [goal]
area:      [agent-building]
...
time:      [14:51] [25-09-26]
agent:     [claude]
worktree:  feat/provider-auth-routes
type:      [feature-request]
area:      [backend]

Web identity M3, daemon side: coding plans over HTTP so a surface can show and manage a node's Claude and Codex subscription logins without the TUI. New crates/ocean-daemon/src/provider_auth.rs adds GET /v1/auth/providers (token-free status per provider: signed_in, expired, signed_out or unknown, with source auth_file, env or codex_cli and access expiry), POST /v1/auth/providers/{provider}/login (runs ocean-oauth begin, returns attempt_id and authorize_url, finishes in a background task), GET and DELETE on /login/{attempt_id} (poll and cancel), and POST /logout. Every route, status included, authenticates through the room operator principal, so a Cookie or foreign Origin is refused on shape and a missing key fails closed. One in-memory attempt per provider; a new start cancels the old one and waits for its task to unwind so Codex's fixed 1455 callback port is free before rebinding. ocean-oauth gains oauth_block_status, logout (atomic removal that never creates a missing file), OAuthProvider::from_label and ALL. The route contract is written as §9 of the program spec under the operator's standing mandate, and §8 records M1's switch from GitHub login to numeric account id after review. The route baseline moves 139 to 144 in discovery, ARCHITECTURE.md and the operator guide. Gates: ocean-daemon 953 tests and ocean-oauth tests green, clippy -D warnings, fmt. Still open: the Surface proxy allowlist with owner-only operator-key injection, and the Coding plans panel.
_________________________________________________________________________________ 14:51 feat/provider-auth-routes
...
Independent review of the M3 provider routes found the auth.json write path racy, and this fixes it before merge. The turn-time refresher read the whole file, spent seconds on the network, then wrote the stale root back, so a web logout could be silently undone, a fresh login clobbered, or a just-rotated Codex refresh token lost; all three writers also shared one temp file name. ocean-providers now owns auth_file_lock() and auth_file_temp_path(); ocean-oauth takes the lock around merge and remove, and oauth_refresh merges into a fresh read under it, touching only a block that still carries the refresh token it spent. The refresher also created its temp file with default permissions, so every refresh left auth.json world-readable; it now creates it 0600. In provider_auth, attempts are spawned and registered under one lock, cancel and logout await the aborted task (a login already past its exchange is recorded succeeded, and logout removes only after that write), a superseding start retries its bind for up to 2s while Codex's 1455 listener closes, and failures reach clients as fixed codes because the flow's text can carry raw provider bodies, attacker-chosen callback text, or the auth path. Spec §9 records all of it. New tests cover the merge never resurrecting or clobbering a block, 0600 after refresh, foreign-Origin refusal, fixed failure codes, and logout racing a live attempt.
_________________________________________________________________________________ 14:58 feat/provider-auth-routes

time:      [15:03] [25-09-26]
agent:     [claude]
worktree:  docs/m2-linking-proposal
type:      [plan]
area:      [docs]
...
time:      [17:09] [25-09-26]
agent:     [claude]
worktree:  feat/observatory-f2-schema
type:      [refactor]
area:      [backend]

Closed Observatory review finding F2: the store schema now matches manifest §4.1 through a versioned, idempotent migration (PRAGMA user_version, ocean-observatory src/migration.rs) that ObservatoryStore::open runs step by step from 0. v1 is the pre-F2 baseline (Task 3 tables plus G3's first_cursor); v2 rebuilds every table by SQLite's create-copy-drop-rename procedure in one IMMEDIATE transaction with foreign_keys off and an empty foreign_key_check required before commit, backfilling event kind/producer_id/visibility/schema_version/created_at from envelope_json, node session/turn/request/producer ids, last_cursor and finished_at from each node's newest surviving event, and edge root_execution_id, and creates all twelve §4.1 indexes. append_event populates every column and moves last_cursor on every event. Documented deviations: no FK from node cursors or watermarks to events (retention prunes events the projection outlives), nullable first/last_cursor for pre-G3 and fully pruned rows, only the child FK on edges (parents are often sessions never observed), and events keep cursor as the primary key plus envelope_json. Eight new migration tests cover G3- and Task 3-shape v0 databases, idempotency, newer-version refusal, unparseable envelopes, new appends, retention after migration and the indexes and FK; a copy of a real 115k-event operator database migrated in 2.5 s with every row preserved. Review doc gained an F2 migration section and ocean-observatory gained an AGENTS.md with the migration rule.
_________________________________________________________________________________ 17:09 feat/observatory-f2-schema
...
time:      [17:27] [25-09-26]
agent:     [claude]
worktree:  fix/observatory-f11-f12
type:      [bug-report]
area:      [backend]

Closed Observatory Task 9 review findings F11 and F12, each item with a regression test. F11: the secret hard link and the token rename now fsync their parent directory (secret_link_and_token_rename_sync_the_parent_directory); a failed ten-minute token rotation increments ocean_observatory_token_rotation_failures_total on /metrics and logs at error with the consecutive count and whether the published token has expired (failed_token_rotation_is_counted_on_metrics); validate_claims refuses expires_at <= issued_at (zero_lifetime_token_is_rejected). The unix-only secret handling stays as is. F12: events?scope= is validated per manifest §7.2, so anything but summary, including the reserved content, is one invalid_scope error frame, the in-band 400 form invalid_cursor already uses (events_rejects_an_unsupported_scope). Snapshot detail=full is documented as reserved and returns the summary shape, because §7.1 defines no metadata field for it (snapshot_detail_full_is_a_reserved_alias_of_summary). Gate 0 decision 7's "interrupted" is reconciled in a new Gate 0 R3 note: the wire enum has no such phase, so the restart sweep writes canceled via execution_phase_changed from the next boot, which operators can tell apart from a cancelled turn's execution_finished (restart_sweep_records_interrupted_as_a_phase_change_to_canceled). Updated the review doc, manifest §7.1/§7.2, operator guide and both crate AGENTS.md files. ocean-observatory and ocean-daemon (977) green; clippy -D warnings, fmt and docs-check clean.
_________________________________________________________________________________ 17:27 fix/observatory-f11-f12

time:      [17:46] [25-09-26]
agent:     [claude]
worktree:  feat/minimizer-m2-checkpoint
type:      [feature-request]
area:      [agent-building]

Implemented the reviewed minimizer M2 design as a characterization-first checkpoint, and nothing is live. M2a (ocean-runtime tests/m2a_characterization.rs) freezes the pre-M2 behavior: the exact Bash envelope, the legacy command-only schema, the spill boundary and exact artifact round-trip, the artifact-read bypass, decorator forwarding (pinning that SpillingTool does not forward concurrency), direct SessionContext defaults, and one permission-gated execution with identical raw live/checkpoint/provider text. M2b adds the mechanism behind SessionContext::command_output_minimization, which defaults off and which no daemon or agent path sets. That covers the additive XOR-validated argv Bash mode (direct execution, schema offered only under the gate), bare-M1-name eligibility for the downcast built-in BashTool only, exact envelope recognition, the 256-byte footer and net-savings rules, PinBudget/ArtifactLease pins (32 entries / 1 MiB per run, skipped by eviction), a sealed ToolExecutionResult via AgentTool::execute_for_run, and loop overrides bound to transcript ordinals so only the provider request clone changes. Leases drop on every return. Provider schema fixtures were added for Anthropic, OpenAI, Codex, and Gemini. Open for review, and not decided here: whether argv mode ships independently of the gate, the concurrency-forwarding repair (it changes live scheduling), and moving ocean-minimizer into default-members. M2c profile wiring has not started.
_________________________________________________________________________________ 17:46 feat/minimizer-m2-checkpoint
...
time:      [18:31] [25-09-26]
agent:     [claude]
worktree:  perf/retained-size-slow-client-measurements
type:      [analysis]
area:      [backend]

Added end-to-end retained-size and slow-client measurements for the ROADMAP "Reliability and scale" item. This is measurement only: no bound, capacity or code path changed. New test-only modules are crates/ocean-daemon/src/retained_size_measurements.rs and crates/ocean-agent/src/session_retained_size_measurements.rs; fast tests assert what they print, and heavy sweeps are ignored. The results are in docs/specs/2026-09-25-retained-size-and-slow-client-measurements.md, linked from ROADMAP (the item stays open) and docs/README. Findings: the agent replay ring holds 10 turns or fewer across all sessions, and its count cap binds below about 830 KiB per tool output. Every bus lags at exactly capacity+1: agent 1024, legacy 1024, room wake 256, access and cursor 64. Through the real router, a stalled agent or legacy SSE client lost about 1,950 of 3,072 events behind one lag frame and was never disconnected. The fast client and the producer were unaffected. A stalled room client was lossless because it re-pages SQLite. Two costs sit outside the 32 MiB replay budget. First, every agent-rail connect deep-clones the whole ring under the lock emit needs (31 MiB, about 1 ms, even for a plain connect). Second, a stalled receiver pins up to 1,024 extra envelope copies, bounded by count and not by bytes. Persisted transcripts stay under 0.53 MiB (200k window) or 2 MiB (1M window), but six full-file saves per turn wrote 64 to 476 MiB over 60 turns. The per-turn MPSC was not measured.
_________________________________________________________________________________ 18:31 perf/retained-size-slow-client-measurements

time:      [18:38] [25-09-26]
agent:     [claude]
worktree:  perf/agent-events-connect-copy
type:      [bug-report]
area:      [backend]
...
time:      [20:32] [25-09-26]
agent:     [claude]
worktree:  feat/extension-stage-a3b
type:      [feature-request]
area:      [backend]

Implemented Stage A slice A3b (manifest §15, §17, §18 A3b, §19.4) over the merged A3a writer. Eight routes join the two A0 reads: credential-free GET /v1/extensions (list) and GET /v1/extensions/{id}/status (cached supervisor projection, never a probe), plus install, trust (preview and apply), enable, disable, update and DELETE remove. Every mutation requires the existing room_operator principal (X-Ocean-Operator, cookie and foreign-origin refusal, 503 absent, 403 invalid) before the strict body is parsed; §15 names no credential class, so this is recorded in the manifest as a builder's reading awaiting a ruling. Adapters live in extension_registry/mutation.rs and call the writer only through spawn_blocking. Responses use §15's pre-commit and committed envelopes; after a commit the route asks the supervisor to reconcile to that revision on its single serialized queue and answers 200 only when that pass completed, including any generation-safe reap, else the committed 202. A committed registry_recovery_required is the 500 with committed:true. The activation descriptor no longer includes the registry revision, so an unrelated commit never restarts a service. A per-package activity ledger (managed tasks, retained cleanup authorities, and in-flight reconciliation passes) backs the writer's update/remove ServiceActivity guard. Disable reaps before 200. Startup recovery runs before the supervisor or router start. Busy readers answer a retryable 503. Git bodies parse strictly but are refused 501 git_connection_pinning_unavailable until A4. The ocean-rs extension CLI gains list/status/install/trust/enable/disable/remove/update with the 0/3/4 exit contract and reads the operator key from the config dir. Recorded open: the credential ruling, §17's no-stop project-scope narrowing (the A2b supervisor still restarts on scope change), and Windows management. Tests: 10 A3b route tests, including a live supervised no-op service, plus 3 supervisor unit tests and 5 CLI tests; a manual CLI smoke against an isolated daemon passed. Pending fresh independent review. A4 is next.
_________________________________________________________________________________ 20:32 feat/extension-stage-a3b
...
Repaired the Knox independent review of PR #502 (request-changes). H1: a retained cleanup failure no longer blocks every pass; stops always run, a failed stop keeps going, and unproven cleanup withholds only a replacement spawn of the same service key and is reported only for its own package; temp cleanup fchmods opened directories owner-writable so 0555 trees such as Go module caches are removable. M1: acquisition, commit, and the reconcile request run in one detached task, so a client disconnect cannot skip reconciliation, and the supervisor retries any blocked or incomplete pass (startup included) with 0.5–30 s backoff. M2: the activation descriptor carries a boot-local per-package generation (the committed revision of its last disable/trust/update/remove), so a disable→enable pair committed before any pass still mints a new epoch and process, while unrelated installs still restart nothing. M3: new behavioral tests cover a retained-cleanup owner refusing update, a real pass held open at a test gate refusing update/remove, and a start_extension_host seam proving recovery completes before the supervisor exists; the text check now rejects commented or spawned calls. Mutation checks each now fail a test: package_stopped forced true, begin_pass removed, recovery spawned, recovery commented out, and the generation zeroed. Low-severity fixes: auth runs before path/body parsing (raw extraction); the in-flight refusal is the distinct retryable reconciliation_in_progress with a CLI hint; envelopes come from what the pass did for the package; status activation_revision equals host_hello; the CLI refuses to send operator.key to non-loopback URLs. Nits: the writer is constructed inside spawn_blocking; the project read is off the executor; the key is opened O_NOFOLLOW|O_NONBLOCK with a uid check and a bounded read; foreign-origin and unconfigured-key auth tests were added. Awaiting delta review; not merged.
_________________________________________________________________________________ 21:03 feat/extension-stage-a3b
...
Folded the Knox delta review of PR #502 (verdict merge-ready) into the branch. The update/remove guard now takes one single-lock ledger snapshot of package ownership and in-flight passes; pass begin/end and every owner change share that lock, so a pass cannot spawn between two separate reads. A generation reset (disable→enable, retrust) now discards restart/backoff/circuit history per §10.4, and the replaced service stops with reason disabled after a disable/remove. The per-package generation bump is recorded in a shared map before the reconcile command is sent, so a send timeout cannot lose it. A dead mutation task, or a panic inside the writer, now answers 500 outcome_unknown with committed:null instead of guessing committed:false; the CLI exits 5 and says to reinspect. Mutation bodies again require Content-Type: application/json, checked after the credential and before the body is read (415). Mutation checks: separate guard reads, a snapshot that ignores in-flight passes, and history kept across a reset each fail a test. ocean-daemon 1033 passed, ocean-cli 20 passed, all gates green. Not merged.
_________________________________________________________________________________ 21:59 feat/extension-stage-a3b

time:      [23:04] [25-09-26]
agent:     [claude]
worktree:  feat/extension-stage-a4
type:      [feature-request]
area:      [backend]

Implemented Stage A slice A4 (manifest §13.2, §18 A4, §19.3-§19.4): pinned public Git acquisition for install and update. The new extension_registry/transaction/git.rs fills the A3a quarantine lease without the state lock, and the §15 routes now send Git sources to it inside the existing detached spawn_blocking task instead of refusing 501. The source must be https://<public-host>[:443]/<path> at an exact 40- or 64-hex id; anything else is invalid_git_source before any permit, quarantine, DNS or process. The host is resolved once through an injectable resolver, and one non-public answer refuses the whole set. Each attempt is a fresh git process pinned with exactly one http.curloptResolve=<host>:443:<checked address>, and the URL keeps the hostname so SNI and certificate checks are unchanged. There is no unpinned fallback. Host git comes only from fixed absolute paths, must be at least 2.37.0 and carry git-remote-https, and runs with env_clear, a fixed environment, and fixed -c hardening: one protocol, no redirects, no proxy, credential helper, extra headers or cookies, hooks at /dev/null, no submodule recursion or LFS filter, and fsck on. Its stderr is discarded. Each git leads its own process group, observed with waitid(WNOWAIT) and reaped only once the group is proven empty; a timeout or the 512 MiB scratch ceiling follows §10.5. Extraction is checkout-free and filter-free (ls-tree plus cat-file --batch, O_EXCL descriptor-relative creates), so a Git source and a local source of the same tree hash identically. Only plain blobs are accepted: symlinks, gitlinks, .gitmodules, .lfsconfig, filter= attributes, and unsafe or colliding paths are refused. Tests: 21 offline A4 tests (local file remote behind the full address check, a scripted fake git, and real git against a loopback listener under a .invalid host proving pin, SNI, no redirect, and no credential) plus one route test. All 13 guard mutations each failed a test. The ignored public-commit smoke fetched octocat/Hello-World at 7fd1a60 over a pinned connection. The manifest A4 status note records the realization choices and six open boundaries. Pending fresh independent review; A5 is next.
_________________________________________________________________________________ 23:04 feat/extension-stage-a4
...
Repaired the Knox review of PR #503 (no blockers; one medium, seven lows, six nits). The Git tree listing now refuses any two paths, or a file and a directory, that share a lowercase+NFC collision key, so README+readme, NFC+NFD café, and DIR/+dir/ fail on every host. The key uses icu_normalizer (compiled_data only), which was already in the lock via idna, so Cargo.lock gains one dependency line and no package. The TreeWriter never-merge guard stays as a second layer with its own test. Local git commands (init, ls-tree, cat-file, the probe) get protocol.allow=never with no allowance; only fetch gets one. transfer.bundleURI=false and fetch.uriProtocols= join the hardening. ls-tree is watched while streaming and its group is killed past the entry cap or 64 MiB (package_invalid). At most four resolver threads can be in flight daemon-wide (a full cap is git_resolution_failed), and at most eight checked addresses are tried. A failing cat-file -t is now git_revision_mismatch, and EAGAIN-class spawn failures are git_fetch_failed rather than 501. On macOS the /usr/bin/git shim is used only when xcode-select or the default developer dirs hold a real git, checked without invoking the shim. An unproven cleanup keeps its acquisition permit until restart. There are 16 new tests, and all 17 new mutations fail a test. On APFS the end-to-end aliasing test needs both collision layers removed, and each layer has its own failing test. The manifest A4 note records the follow-up and a seventh boundary: local sources do not apply the aliasing rule. Not merged.
_________________________________________________________________________________ 23:36 feat/extension-stage-a4

time:      [00:04] [26-09-26]
agent:     [claude], [claude-opus-5-5], [flux]
worktree:  fix/extension-a4-followups
type:      [bug report]
area:      [backend]

Repaired the independent delta review of Stage A4 (two lows, one nit) on a branch cut from feat/extension-stage-a4. The Git tree collision key is now per-component NFC(full case fold(NFD)) through icu_casemap 2.2, a new package from the same ICU4X 2.2 release as the already-locked icu_normalizer (unicase, the only folding crate in the lock, gives equality but no folded string). So straße/strasse, final sigma, the fi ligature and U+0345/iota now collide on every host as they do on APFS, while dotless ı and i stay distinct. Each component is folded once and prefix keys are built incrementally. A git process group whose cleanup cannot be proven no longer strands its acquisition permit until restart: the unreaped group and the permit go to a detached waiter that re-checks with 50 ms to 10 s backoff, never signals, and releases the permit (reaping the leader) only once the group is proven empty. If the waiter cannot spawn, the permit is kept rather than released early. locate_git now delegates to a testable locate_git_from shim gate. The new tests cover the fold cases plus a negative ı/i case, an end-to-end stranded-permit test (held while a member lives, released after, capacity restored), a direct waiter test, and the shim gate at its call site. Of 11 mutations tried, 10 fail a test. The survivor drops the strand on the lost-ownership path, which no test can provoke. The manifest A4 note records the follow-up. Not merged.
_________________________________________________________________________________ 00:04 fix/extension-a4-followups
...
Implemented Stage A slice A5, the integrated Stage A gate from manifest §18 and §20. It is tests only: no route, schema, persisted field, frame, lifecycle kind, or runtime behavior changed. The new extension_registry/mutation/stage_a_gate.rs drives the real §15 routes, the A3a writer, the supervisor, the lifecycle dispatcher, and ordinary fake-provider turns through agent_turn. They run over one real no-op shell service that persists its cursor under data/, records every frame it receives, and forks a cooperative grandchild. The gate covers the §20 steps. It proves an offline local install and non-executing reads, trust that neither enables nor starts, host identity, the exact minimal environment, and file-identity-checked assigned roots. New and resumed sessions produce exactly the scoped metadata kinds, with no prompt or cwd bytes and a client shape identical to a no-extension baseline. Disable, interval events, and a stale re-enable get activation_changed with no interval disclosure. A restart through start_extension_host gets a new boot id and boot_changed, and keeps data/. Same-epoch resume after a process failure replays exactly the unprocessed facts. Crash-loop backoff was measured at 1 s and 2 s, the circuit opens with every leader and grandchild dead, and disable then enable retries it. Active update and remove are refused. Every §12.4 retention cell holds through remove, identical reinstall, retrust, and purge. A pinned Git install carries the local digest, and adjacent sources are refused. A bound secret is absent from every registry file, response, status, event, and argv. A stalled service never delays turns. Gap-closure tests cover partial §19 rows: every reset reason, control-lane order, queue count and byte bounds with computed replay_available, the rolling window, stable reset (an ignored real-time test, run once and passed in 310 s), handshake failures on the production runner, missing secret at run level, observer frames that try to publish, allow-session and deny on the real permission policy, reader grant order, trust, service, and binding authority, state and manifest caps, a hardlinked payload, stale revisions, filesystem declarations, the CLI sending a mutation exactly once, Observatory isolation, and the single ordered daemon_started and daemon_stopping producers. Each of 24 production mutations failed a named test. docs/specs/2026-09-26-ocean-extension-stage-a5-acceptance-evidence.md maps every §19 row and §20 step to its tests. Stage A is not accepted: it still needs operator rulings on the mutation credential class, §17 project-scope disable of a shared service, Windows R5, and local-source path collisions, plus the operator's own acceptance. Draft PR only; not merged.
_________________________________________________________________________________ 01:07 feat/extension-stage-a5
...
Repaired the Knox review of PR #505 (one high, three mediums, six lows, four nits). All repairs are to evidence honesty; production code is unchanged. High: the stalled-service gate now creates real backpressure. 160 concurrent turns overflow the 256-frame queue and the pipe of a service that never reads. The gate proves lag_count > 0 and a protocol_violation connection failure no earlier than 2 s after the burst began and within 3.5 s of its end, with every turn bounded and the group reaped. Mediums: a live permission and tool round-trip through agent_turn on the fake-tool model and the real policy waiter. The service receives the exact seven-kind sequence with host UUIDs and no path, content, or runtime id, and a denied turn has no tool facts. Live project-scope widening through the real project-create route proves a new epoch and floor, an activation_changed reset, and no interval disclosure. The shell §20 fixture is listed as operator ruling R-5, and every recorded host frame is now strictly decoded and re-encoded byte-exactly. Assigned-path checks now compare inodes the child resolves, so the Linux /proc/self/fd spellings work. Lows and nits: backoff slack is tightened to +500 ms, the no-timer wait is now 5 s, the grant-diff row is marked partial, there is a live Git update in the gate, the secret stderr redaction is counted, and the exactly-two turn_finished is labeled an observation. Also: an exact 1 MiB boundary test, a panic-safe env restore in the CLI test, and SVC5 no longer claims boot_changed. Eight more mutations (M11b, M25–M28, M8b, M30, M31) each fail a named test. The gate and a5_ suites ran three times clean. Not merged.
_________________________________________________________________________________ 01:56 feat/extension-stage-a5
...
time:      [03:44] [26-09-26]
agent:     [claude], [claude-opus-5-5], [Flux]
worktree:  test/extension-stage-a-open-items
type:      [feature-request]
area:      [testing]

Closed the Stage A5 evidence record's §6 open items after #505. O-1: a live gate test blocks the fake-tool write on a FIFO, cancels the turn through the real request route, and proves the bridge publishes tool_finished cancelled. O-2: the oversized branch is proven unreachable by a worst-case bound test over all ten metadata variants, and its mapping is unit-tested; the mapping moved unchanged into a named frame_error_diagnostic. O-3: a gate test opens the circuit and retries it by daemon restart and by a new trusted digest. The new-digest retry is reachable only after a disable. O-4: the cwd is proven to stay on the anchored directory under path replacement. O-5: the secret gate runs under a TRACE tracing capture on every runtime thread, and a crash-matrix scan shows no secret on disk at any journal step. Panic payloads stay structural. O-6: a ping-timeout grandchild is proven reaped. O-7: all four acquisitions are held while the lock stays free and list, inspect, and doctor answer over HTTP. O-8: every grant-diff shape is proven to preview stably. O-9: the stalled gate now runs a real permission round-trip against the full queue. O-10: a new default-off ocean-runtime test-support feature, enabled only as a daemon dev-dependency, lets tests point the fake-tool write at a per-test path; production still writes the fixed path. Every new guard is mutation-checked (M32–M42). Added O-11 for an operator ruling: §7.1/§9.2 do not say whether the 2 s stdin deadline is per frame or per connection. It is per frame today, and a characterization test pins that. No production bug was found, and no wire, route, schema, or persisted format changed. The evidence doc, the manifest A5 note, and the root, daemon, and runtime AGENTS were updated. Draft PR, not merged.
_________________________________________________________________________________ 03:44 test/extension-stage-a-open-items
...
worktree:  [cc/rooms-dod-next]
type:      [review]
area:      [docs], [analysis]

Audited every Rooms DoD line against code rather than the doc and wrote down what moved. 0.5 is met as written: the operated daemon's /health reports f458f2ea, which contains the #436 to #439 paging fixes and is recorded in the S0 deploy entry, though it is 17 days behind main. 0.6's engineering half is shipped (deploy/ocean-daemon.sh reads the 0600 federation.env or Keychain item on every launchd start, never launchctl setenv; scripts/federation-loader.test.mjs 13 pass); federation is still off on the operated daemon, which is the user's to turn on. 4.3 is done as written: the close route, room_maintenance retention and orphan GC, and Bedrock's idle reaper all exist with named tests, with two limits noted (retention defaults to never; the surface has no close control). 4.5's runbook half is present; the rehearsal is still owed. 5.3 is partial: this repo cannot read private ocean-bedrock without a token, so ocean-bedrock #163 (draft) reds Bedrock's CI when it publishes an action this repo's public vendored copy lacks. Also replaced OPERATIONS.md's "Reading the bridge without metrics" subsection, which still said /metrics had no room counters after 4.1 shipped them, with the real families and the /health rooms and room_maintenance cards. Docs only.
_________________________________________________________________________________ 05:47 cc/rooms-dod-next
time:      [07:01] [26-09-26]
agent:     [claude], [opus 5.5], [flux]
worktree:  [fix/daemon-room-member-lane-csrf]
type:      [bug report]
area:      [backend]

Closed the no-preflight cross-site write hole on the daemon. CORS only governs preflighted requests and response reads, so a web page the operator visited could POST /v1/rooms/persistent/{key}/close?actor_id=<member> (no body, no custom header) and any other simple-request write: raw-body attachment upload, room_update, read-cursor PATCH, outbox retry, voice STT, the call webhook, livekit-token with its optional body, and every body-less POST/DELETE. A new cross_site_write.rs middleware, mounted once in app_router inside CORS, refuses every non-GET/HEAD/OPTIONS request that carries Cookie (403 ambient_credential_rejected) or a present Origin/Referer outside the CORS trust set (403 foreign_origin_rejected); an absent Origin passes exactly as room_operator::check_origin allows it, so the CLI, MCP, TUI, ACP, the surface proxy (which rebuilds upstream requests without browser headers) and the macOS Tauri webview keep working. cors.rs now exposes one BrowserOrigins set that both layers are built from. 17 tests, 12 mutations all killed. Operator guide, ecosystem contract, ARCHITECTURE, daemon AGENTS, and DoD 3.1 updated.
_________________________________________________________________________________ 07:01 fix/daemon-room-member-lane-csrf
time:      [07:39] [26-09-26]
agent:     [claude], [opus 5.5], [flux]
worktree:  [fix/daemon-room-member-lane-csrf]
type:      [bug report]
...
Folded Knox's #510 review into the branch. Correction to the entry above: the write guard covers direct cross-site writes only. The surface proxy strips Origin, which is a compatibility fact and not a protection, so a write laundered through an OCEAN_SURFACE_AUTH=off proxy still passes until ocean-surface #230 lands. The operator guide, daemon AGENTS, ecosystem contract and DoD 3.1 now say so. Added host_guard.rs, the DNS-rebinding stop: every request on every method, preflights included, must name localhost, a loopback IP literal, the OCEAN_BIND IP (any IP literal when bound unspecified), an OCEAN_ALLOWED_ORIGINS host, or an OCEAN_ALLOWED_HOSTS entry, or it gets 421 host_not_allowed. A rebinding Host can no longer read a home file through GET /v1/fs/file. The Host allowlist sits outside CORS. Also: origin_of strips userinfo, which closed a Referer bypass (http://localhost:8080@evil.example passed the loopback check); a literal null is dropped from both origin allowlists. New tests kill the non-room-route, DELETE, and guard-outside-CORS mutants behaviourally (CORS headers on a 403). The cookie tripwire was checked against the side panel: the proxy cookies are SameSite=Strict and direct cross-origin calls never attach cookies, so no narrowing was needed. Still open: any loopback port and any chrome-extension are trusted for writes.
_________________________________________________________________________________ 07:39 fix/daemon-room-member-lane-csrf
time:      [08:02] [26-09-26]
agent:     [claude], [opus 5.5], [flux]
worktree:  [fix/daemon-room-member-lane-csrf]
type:      [bug report]
...
time:      [09:37] [26-09-26]
agent:     [claude], [opus 5.5], [sage]
worktree:  docs/bounded-turn-event-channel
type:      [plan]
area:      [backend]

Wrote a PROPOSED design for the runtime-to-daemon per-turn event channel: docs/specs/2026-09-26-bounded-turn-event-channel-proposal.md. It is docs only, the operator answers yes or no, and the ROADMAP line and docs/README point at it. The "channel" is two unbounded hops. H1 is runtime to ocean-agent: its consumer clones every event into H2 and runs checkpoint saves synchronously. H2 is ocean-agent to the daemon bridge. H2 carries AgentEnd (the whole history) and TurnCheckpoint, and the bridge discards both. TurnFinished and permission prompts never enter either hop. I measured the H1 stall with a scratch release test (not committed), timing cap_session_history plus session::save at steady state: p50 9–11 ms, max 22 ms. I re-ran the plain-connect lock hold at 320 ns. Live tool output is large only on voice turns, where spill is off and bash or web_fetch can carry 2 MiB, and for images; the other profiles spill anything over 24 KB down to a 16 KB head. The recommendation is one TurnEventChannel type at both hops, 8 MiB and 1,024 items per hop. When full, the producer waits (backpressure) and never drops. Adjacent same-kind deltas merge up to 64 KiB. An oversized event is still admitted when the queue is empty, and a send gives up when the turn is cancelled. H1 stops forwarding the variants the bridge discards. Nothing changes on the wire. The plan is six slices, with mutation-checked guards for "facts never dropped" and "memory bounded".
_________________________________________________________________________________ 09:37 docs/bounded-turn-event-channel
...
Added two dated corrections (2026-09-26) to docs/specs/2026-09-25-retained-size-and-slow-client-measurements.md. The original text stays in place, with inline notes C1 and C2 and a Corrections section at the end. C1: the turn bridge calls AgentEventBus::emit directly (crates/ocean-daemon/src/main.rs:6985, 7010 onward), not emit_agent, so bridged runtime events never reach the legacy rail. Only the daemon's own SessionCreated (6959), TurnStarted (6970), TurnFinished (6061) and advisor emits (7597) go through emit_agent (9098). The doc's legacy-rail figures are therefore an upper bound for production. C2: the roughly 1 ms connect lock hold now applies only to ?replay=1&session_id=. A plain connect holds the lock for about 320 ns since #496, re-measured at bd2f9abb.
_________________________________________________________________________________ 09:52 docs/bounded-turn-event-channel
time:      [09:40] [26-09-26]
agent:     [claude], [opus 5.5], [sage]
worktree:  [docs/rooms-member-lane-auth]
type:      [plan]
area:      [backend]
...
time:      [10:54] [26-09-26]
agent:     [claude], [claude-opus-5-5], [flux]
worktree:  feat/federated-actor-mismatch-metric
type:      [feature-request]
area:      [backend]

Added observability for member-lane threat T4 without changing behavior: a new seventh room metric family, ocean_room_federated_actor_substituted_total{route} with routes post, workspace_read and workspace_command, mirrored on /health as rooms.federated_actor_substituted with a total. persistent_rooms::observe_federated_actor counts a federated post after a successful enqueue, and a gated non-Agent workspace call just before it is forwarded, when the caller's trimmed non-empty author_id or ?actor_id= is neither the credential's local_human_member_id nor the node's /v1/identity id. Matching against both matters because no local client ever holds the Bedrock id, so the naive comparison would count every ordinary post. Each increment logs one warn with the room key, the route, whether the id is on the local roster and whether a node identity is set, and never the ids. Remove, invite mint and redeem, register-agents, read-cursor and outbox retry read no caller identity (deny_unknown_fields bodies, no query), and invoke checks invoked_by rather than substituting it, so none of those is instrumented. Tests cover both post outcomes, a refused post, a Local post, all three workspace outcomes and the classifier edges, and ten mutations each went red. OPERATIONS "Reading the bridge", the operator guide, the proposal (S1 note), DoD 3.1 and the daemon AGENTS metrics bullet were updated.
_________________________________________________________________________________ 10:54 feat/federated-actor-mismatch-metric

time:      [11:48] [26-09-26]
agent:     [claude], [claude-opus-5-5], [flux]
worktree:  perf/turn-channel-slices-1-2
type:      [refactor]
area:      [backend]

Landed slices 1 and 2 of the bounded turn event channel proposal, pending review. No bound, backpressure, coalescing, channel type or metric was added; those are slices 3–6 and wait for the operator. Slice 1 makes the proposal's scratch H1 save-stall measurement permanent as an #[ignore]d release-mode test (checkpoint_save_stall_at_steady_state), timing cap_session_history plus session::save plus fsync at exactly 539,534 B and 1,029,539 B; it measured about 9–13 ms p50 and at most 19 ms, matching the proposal. The stalled-consumer peak-bytes harness from slice 1 is not built. Slice 2 changes ocean-agent's H1 forwarder so it moves events into the event sink instead of cloning them, and stops forwarding the seven variants the daemon bridge discards, including AgentEnd (the whole history) and every TurnCheckpoint delta. An audit found the daemon bridge is the only production sink and relies on none of them. A daemon golden test pins the client-visible sequence of a fake-tool turn and passes before and after the change; an ocean-agent test proves the dropped kinds no longer reach the sink while stdout, stderr and the transcript stay the same. Four mutations were checked. The proposal doc §8, both crate AGENTS files and the bridge comment were updated.
_________________________________________________________________________________ 11:48 perf/turn-channel-slices-1-2
...
Folded Knox's review of #517 into the same branch; still unmerged. AgentEvent::is_wire_relayed() in ocean-runtime is now the one exhaustive relay classification. The ocean-agent forwarder uses it with no catch-all of its own, and the daemon's hand-kept classify_agent_event mirror is gone. The bridge body moved verbatim into run_turn_bridge, so a parity test can send all 19 variants through the real bridge and assert it agrees with is_wire_relayed. Mid-turn checkpoint persistence was unpinned, because deleting the save survived the suite. It is now pinned deterministically: a permission policy cancels the turn before the tool runs, a failed turn does no final save, and the stored transcript must still hold the round-1 call and result. The daemon golden now checks call_id pairing and result.ok, and gains denied-permission and cancelled-turn variants that pin the lifecycle facts. A failing scripted-provider turn was not added, because no keyless fake model fails. The ocean-agent env var now sits behind a lock and a restore guard. Five new mutations each went red where expected. Proposal §8 and the runtime, agent and daemon AGENTS files were updated.
_________________________________________________________________________________ 12:17 perf/turn-channel-slices-1-2

time:      [12:58] [26-09-26]
agent:     [claude], [claude-opus-5-5], [flux]
worktree:  perf/turn-channel-queue-measurement
type:      [refactor]
area:      [backend]

Built the stalled-consumer harness that slice 1 of the bounded turn event channel proposal left unbuilt. It is measurement only: no production behavior changes, and no bound or backpressure was added. turn_channel_queue_peaks_under_stalled_consumers is an #[ignore]d ocean-agent test. It runs the real prompt, run_prompt and agent-loop path on a multi-thread runtime, with a paced scripted provider, the real bash tool, the real H1 consumer and its checkpoint saves, and an H2 consumer that stalls for a fixed time before draining. It reconstructs exact peak queued events and bytes per hop from rx.len() at each dequeue, with bytes counted by the proposal's §4 estimate. The only production-file hooks are two #[cfg(test)] AgentRuntime fields: a turn provider and the H1 dequeue probe. Across three release runs, H1 never held more than 6 events, and a checkpoint save at either proposal size left 2–5 queued. H2 held exactly what arrived while the bridge was stalled: 406 events and 49.2 KiB for a whole 3 s turn. Four 2 MiB spill-off bash results under a whole-segment stall reached 8,394,780 B, 6,172 B over the proposed 8 MiB budget. That is the only case where either bound binds; the 1,024-event count was never approached. Proposal §8, measurements doc §7 and the inventory row, and the ocean-agent AGENTS test list were updated.
_________________________________________________________________________________ 12:58 perf/turn-channel-queue-measurement
