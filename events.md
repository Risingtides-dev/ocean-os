...
_________________________________________________________________________________
time:      [02:57am] [06-24-26]
agent:     [codex] [gpt-5]
worktree:  main (cross-repo update)
type:      [operator-maintenance]
area:      [surface/acp]

Updated local operator repos to current GitHub tips: ocean-os main, ocean-surface
main after forced remote update, ocean-agents main, and ocean-bedrock default
branch master (repo has no main branch). Rebased the local Cursor/VS Code ACP
surface integration onto the new ocean-os main: ocean-acp now honors
OCEAN_ACP_CLIENT_TYPE so the sibling Cursor extension can report acp-vscode,
and ocean-agent recognizes acp-vscode as the ACP surface with a compiled
editor-surface fallback when no assistants/ACP profile is installed.

Preserved pre-update local state with backup branches/stash; no parked
untracked operator folders were deleted.
_________________________________________________________________________________
