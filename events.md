...
time:      [19:45] [20-07-26]
agent:     [claude] [fable 5]
worktree:  fix/skills-fetch-symlink-retarget
type:      [bug-report]
area:      [backend]

Resolved the long-deferred librarian symlink-retarget security finding (flagged separate-disposition in the 07-15 turn-preparation extraction manifest and in ocean-daemon AGENTS.md, blocking any librarian extraction). skills_fetch resolved a path from the TTL-CACHED skill index and then read it with read_to_string, which follows symlinks — so an entry that was a regular file at walk time and a symlink at fetch time became an arbitrary file read. Severity is higher than "local write access" implies because SkillRoots::for_cwd adds the REPO-LOCAL <cwd>/skills root: the symlink author need not be the operator, an untrusted repo can ship one, and the daemon would return whatever it targets over HTTP. Fix re-validates at READ time with symlink_metadata and fails closed on symlink or non-regular file, keeping the existing TOCTOU Unreadable arm for genuine races. HONEST SCOPE: confirmed by code inspection and pinned by regression; I did NOT produce a live end-to-end exploit — the daemon's index cache made timing a demo unreliable, and I stopped rather than claim a proof I do not have. The loader already skips symlinks during the walk, so this closes the cached-then-retargeted path specifically. 537 daemon tests green, clippy zero.
_________________________________________________________________________________
