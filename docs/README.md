...
- [`specs/2026-08-16-ocean-rooms-distributed-workspace-architecture.md`](specs/2026-08-16-ocean-rooms-distributed-workspace-architecture.md) — operator-approved 2026-08-17 concept for Rooms as a distributed authorization and work context: room-scoped agent bindings, member-contributed folders and compute, a logical resource namespace spanning Ocean nodes, Tailscale-backed direct operations, local enforcement, and extension-owned multi-node orchestration. [`Gate 0`](specs/2026-08-17-ocean-rooms-gate0-decisions-and-threat-model.md) is accepted. The [`Phase 1 room-agent authorization manifest`](specs/2026-08-25-ocean-rooms-phase1-room-agent-authorization-manifest.md) was operator-accepted 2026-08-25; its exact Ocean OS and Surface candidates passed independent review and local/GitHub gates, then landed through Ocean OS PR #425 and Ocean Surface PR #177 on 2026-08-31. Its §15 closures remain binding. Stage 2 contributed folders and every later phase remain closed until their own implementation manifests are accepted.
- [`specs/2026-09-25-ocean-web-identity-and-node-linking-program.md`](specs/2026-09-25-ocean-web-identity-and-node-linking-program.md) — operator-approved 2026-09-25 program for GitHub sign-in on `ocean.agentsworld.org`, self-serve node linking, web management of a node's Claude/Codex coding-plan logins, and Rooms under one identity. Authorizes M1 (proxy GitHub sign-in) only; M2 and M3 each need an accepted route/contract manifest before daemon or hub routes change. The proxy and coordinator never hold model credentials.
- [`specs/2026-09-26-rooms-member-lane-authentication-proposal.md`](specs/2026-09-26-rooms-member-lane-authentication-proposal.md) — PROPOSED 2026-09-26, not accepted. The member-lane half of Rooms DoD 3.1: a daemon-verified member principal (the operator key, or the M2 `X-Ocean-Link` plus `X-Ocean-Acting-Member`), self-or-owner leave and remove, and federated rooms pinned to the node human. It includes the trust chain, the threat list, the rollout slices, and five yes/no questions. It authorizes nothing.
- [`specs/2026-09-01-ocean-rooms-definition-of-done.md`](specs/2026-09-01-ocean-rooms-definition-of-done.md) — the production-grade finish line for Ocean Rooms across ocean-os, ocean-surface and ocean-bedrock, written 2026-09-01 from three read-only assessments. It records member-visible outcomes, current evidence, owners, and the executable checks or remaining check gaps known for each criterion. It drives the Ocean Loop's priority; it authorizes no implementation by itself.
- [`specs/2026-07-16-ocean-minimizer-command-capture-runtime-integration-design.md`](specs/2026-07-16-ocean-minimizer-command-capture-runtime-integration-design.md) — reviewed design for conservative minimization of explicitly tokenized Bash argv in active-run provider requests; live events and durable session history stay raw, while exact recovery artifacts remain pinned for the projection lifetime. Its M2a characterization and M2b runtime mechanism are implemented behind a default-off `SessionContext` gate that no profile sets; M2c profile wiring (what would make it live) is not started.
