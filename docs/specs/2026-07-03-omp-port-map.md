...
| `ocean-minimizer` | M1 fixed filters over already-tokenized invocations; no TOML/config, artifact persistence/reference, or footer. Its only consumer is `ocean-runtime`'s M2 mechanism behind a default-off gate no profile sets | `crates/pi-shell/src/minimizer/` |
...
- **bash** — persistent session; `cwd`/`env` params instead of `cd`/exports; optional PTY;
  `async: true` → job ID; auto-background after 60s. OMP's **output minimizer** combines
  per-command filters, TOML definitions, and a caller-owned raw-output artifact reference.
  Ocean M1 implements only a standalone fixed-filter library for cargo/git/gh/npm/npx/pytest;
  command capture and any artifact-backed integration remain a separate checkpoint.
...
- **pi-walker** (~4,600 LOC) — **implemented as standalone `ocean-walker` M1, unwired.**
  Pure-Rust traversal engine with hand-rolled platform fast paths: `getdents64` (Linux),
  `getattrlistbulk` (macOS), `NtQueryDirectoryFile` (Windows). Builder API, per-level
  gitignore stack, heartbeat closures for cancellation, ranked top-N
...
- **pi-shell** (~30k LOC incl. filters) — **fork and SPLIT; don't take whole.**
  ...
  defs, and a 4MiB raw fallback. OMP returns original text to its caller for optional artifact
  persistence; the minimizer itself does not persist an artifact or append a footer. Ocean M1
  intentionally ports a smaller dependency-free fixed subset with no TOML/config/artifacts and
  remains standalone in `ocean-minimizer`.
...
## Implementation status audit (2026-07-16, walker M1 refreshed 2026-07-19)
...
| W3 — context economy | **Partial** | Artifact spill is live. `ocean-agent::compact_history` deterministically elides older tool bodies and `ocean-runtime::trim_to_context_window` provides the hard suffix bound. `ocean-ast` implements structural summaries as a standalone crate. `ocean-minimizer` M1 implements fixed conservative filters as a standalone, dependency-free crate, and the reviewed M2 design freezes an explicit-Bash-argv, active-provider-only, turn-pinned-artifact boundary while raw events/checkpoints/session history remain authoritative. The M2a characterization and M2b runtime mechanism (additive Bash `argv` mode, exact envelope recognition, sealed provider-only projection bound to transcript ordinals, `PinBudget` leases) are implemented in `ocean-runtime` behind `SessionContext::command_output_minimization`, which defaults off and is set by no daemon/agent path. `ocean-walker` and `ocean-search` M1 implement standalone traversal and bounded typed trusted-root search without production wiring. | Minimizer M2 is not live: M2c profile wiring is not started and awaits review of the M2b checkpoint's open decisions (argv exposure policy, wrapper `concurrency` forwarding, `default-members`). Live walker/search adoption remains pending and requires a separate path-authorization boundary. There is also no BM25 tool discovery, promote/prune/shake/summarize pipeline, protection matchers, live general AST-read wiring, or conversational checkpoint/rewind. |
...
- `ocean-walker` M1 implements the pinned donor traversal, symlink, filtering, ordering, and
  parallel/serial behavior as a standalone workspace crate, with bounded correctness fixes:
  exact native owned-path identity, explicit fresh/cache provenance, post-scan cache age,
  absolute lexical cache/invalidation keys, generation-linearized publication/invalidation,
...
1. Review the minimizer M2a/M2b characterization-first checkpoint (default-off, not live) and rule on its open decisions; only then implement M2c profile wiring. Do not describe minimization as live before M2c lands.
...
Same rule everywhere: minimizer filters for Ocean's commands
(cargo/git/gh/npm/npx/pytest), grammars for Ocean's languages, and web-search providers Ocean
would use. M1 keeps minimizer support as reviewed compiled code rather than importing OMP's TOML
or user-config surface; later extensibility requires its own design.
...
3. **W3 — context economy: partial.** The minimizer M1 is built; the M2a/M2b runtime checkpoint
   exists behind a default-off `SessionContext` gate that no profile sets, so minimization is
   not live until the separately reviewed M2c profile wiring. The standalone walker
