...
2. Move leaf concerns first:
   ...
   - **Published (2026-07-16):** characterization `f6e8efe` froze full presentation/no-op bytes, real-file body/path non-disclosure, environment and whitespace-prompt behavior, cwd/root and brief propagation, cache/ranking inside `spawn_blocking`, timeout/outcome branches, exact helper warnings, wired module authority, and the ordinary prompt, asynchronous request, and agent-turn call-site prerequisite/application/runtime order. The exact 228-line implementation boundary then moved into the 246-line private `src/longhouse_turn_preparation.rs` owner; `main.rs` retains all three call sites and their caller-cwd, permit, acknowledgement, event, runtime, and prompt-layer order. Focused groups, all 351 daemon tests, 118 Longhouse tests plus one host-dependent ignore and one doc test, workspace test compilation, both supported features, compatibility, Rust 1.88 MSRV, canonical local CI, formatting/diff checks, mechanical comparison, two fresh extraction reviews, hosted macOS/Ubuntu/MSRV/cargo-deny CI, and PR #299 merge `9095d5a` passed with no unresolved medium-or-higher issue. HTTP adapters, librarian/spec, governance, calls, and broader turn/SSE orchestration remain excluded; delegated loader path logs, unsanitized advisory text, and uncancelled-task/cache-lock amplification remain documented separate risks. Manifest: `docs/specs/2026-07-15-ocean-daemon-longhouse-turn-preparation-extraction-manifest.md`.
   - **Published (2026-07-17):** fresh mapping at `5b9e23a` narrowed the first governance move to only `longhouse_demo`, `longhouse_topics`, and `longhouse_topic`; final rebased characterization `c443df9` freezes empty/full HTTP envelopes, the exact detached 17-event public wire/content/ID/tally sequence, immediate acknowledgement, projection-before-publication with no lock across publish/await, poison asymmetry, one registry shared by runtime extensions and `AppState`, parent-owned routes/SSE, and whole-owner authority exclusions. Commit `0199d57` moves the exact 252-line definition/comment boundary into the 273-line private `src/longhouse_topics.rs` owner. Focused groups, all 456 pre-publication-rebase daemon tests, 168 Longhouse tests plus one host-dependent ignore and one doc test, workspace test compilation, both daemon features, daemon all-target Clippy, compatibility, pinned Rust 1.88 MSRV, canonical local CI, formatting/docs/diff checks, mechanical comparison, and fresh correctness plus security/lifecycle reviews passed with no unresolved medium-or-higher issue; after rebasing through `a517f6c`, focused groups and all 483 daemon tests passed; after reconciliation through `ad391f3`, focused groups and all 488 daemon tests passed; and after the latest rebase through `9177598`, mechanical comparison, focused groups, all 491 daemon tests, daemon Clippy, formatting, docs, and diff checks passed again. Hosted macOS/Ubuntu/MSRV/cargo-deny CI and PR #305 merge `9676b18` passed. Real convene remains explicitly deferred to the second security-sensitive governance manifest because it directly couples ready-model filtering and asynchronous council orchestration to durable title grant/bind and raw-token response delivery; no helper trait, substate, public seam, or broader governance authority entered the owner. Manifest: `docs/specs/2026-07-16-ocean-daemon-longhouse-topic-projection-extraction-manifest.md`.
   - **Published (2026-07-27):** fresh mapping narrowed the security wave to the existing five claim/revoke/recall/breach/board HTTP adapters only. Comment-only checkpoints corrected false authenticated/unforgeable/durable claims before characterization `f1971c0` added seven parent-owned tests freezing exact methods/extractors/envelopes; claim identity-first secrecy, one-shot lifecycle, captured-log/event/projection/reopened-DB non-disclosure; no-auth manual revoke, omitted/zero/one threshold recall, successful cleanup plus retained non-live tally, three-report breach and post-close 200/zero-strike behavior; persisted titles/strikes/revocations versus memory-only recall; board validation/fold-before-publish/kind/poison behavior; title/recall poison; and exact 13-item/import/visibility/source authority. Test-only import-contract correction `0e7a4dc` is the accepted rollback. All 500 daemon tests, 168 Longhouse tests plus one host-dependent ignore and one doc test, denied-warning daemon Clippy, formatting/docs/diff, and fresh correctness plus security/lifecycle reviews passed with no unresolved medium-or-higher issue. The authorized `longhouse_governance_control.rs` candidate uses only existing parent-private handles and owner algorithms; no trait, substate, registry, dependency, public seam, auth redesign, or executable test seam is permitted. Pre-existing local-route trust, caller-supplied recall voter IDs, blocking SQLite, abandoned recall retention, latent request `Debug`, post-close breach 200, and board poison divergence remain preserved risks requiring separate approval to change. The exact 13-definition boundary now lives in private `src/longhouse_governance_control.rs`; current-main reconciliation preserved its mechanics exactly and passed all 592 daemon tests, denied-warning daemon Clippy, formatting/docs/diff checks, and fresh correctness/security review with no unresolved medium-or-higher issue. Real convene stays in `main.rs` because provider readiness, awaited council execution, projection/event delivery, durable title grant/bind, and raw-token response delivery cannot yet be deterministically characterized without an unauthorized seam. Manifest: `docs/specs/2026-07-19-ocean-daemon-longhouse-governance-control-extraction-manifest.md`.
3. Separately manifest real convene/title grant-bind before calls and any genuinely remaining state registry/control-plane boundary.
