...
2. Move leaf concerns first:
   ...
   - **Manifested (2026-07-19):** fresh mapping at `afffd1d` narrowed the next security wave to the existing five claim/revoke/recall/breach/board HTTP adapters only. The exact 13-definition `longhouse_governance_control.rs` candidate can use existing parent-private `AppState::{titles,revoker,recalls,longhouse,agent_events}` handles without a new trait, substate, registry, dependency, or public seam. A separate comment-only source-accuracy checkpoint replaced false authenticated/unforgeable/durable claims with the actual no-auth-extractor local-route posture, caller-supplied recall UUID/threshold-one behavior, accepted-report breach escalation plus the post-close 200/zero-strike response, and in-memory/poison-divergent board semantics. Extraction remains unauthorized until parent-owned characterization freezes complete HTTP envelopes/methods, claim/revoke/recall/breach/board lifecycle and persistence, title non-disclosure, lock poison, board fold-before-publish/poison behavior, memory-only recall, and exact source authority. Pre-existing local-route trust, caller-supplied recall voter IDs, blocking SQLite under the title mutex, abandoned recall retention, and board poison divergence remain documented behavior-neutral risks; changing them requires separate approval. Real convene stays in `main.rs` because provider readiness, awaited council execution, projection/event delivery, durable title grant/bind, and raw-token response delivery cannot yet be deterministically characterized without an unauthorized seam. Manifest: `docs/specs/2026-07-19-ocean-daemon-longhouse-governance-control-extraction-manifest.md`.
