...
2. Move leaf concerns first:
   ...
   - **Manifested (2026-07-16):** fresh mapping at `5b9e23a` narrowed the first governance move to only `longhouse_demo`, `longhouse_topics`, and `longhouse_topic` in proposed private `src/longhouse_topics.rs`. The scripted producer and read adapters share the existing `AppState::{agent_events,longhouse}` observability boundary, while route composition and the single registry shared with runtime extensions remain in `main.rs`. Real convene is explicitly deferred to the second security-sensitive governance manifest because it directly couples ready-model filtering and asynchronous council orchestration to durable title grant/bind and raw-token response delivery; no helper trait, substate, or public seam will be invented to split it. Characterization must freeze the detached 17-event demo, projection-before-publication ordering, poison asymmetry, topic HTTP envelopes/order, and shared-handle identity before movement. Manifest: `docs/specs/2026-07-16-ocean-daemon-longhouse-topic-projection-extraction-manifest.md`.
