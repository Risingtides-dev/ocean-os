...
2. Move leaf concerns first:
   ...
   - **Extraction authorized (2026-07-16):** proposed a separate private `src/longhouse_turn_preparation.rs` owner for only the fresh default-on opt-out gate, deterministic advisory rendering/application, and cached read-only `TurnPrep` selection inside the existing blocking task and 250 ms deadline. Characterization `f6e8efe` freezes full presentation/no-op bytes, real-file body/path non-disclosure, environment and whitespace-prompt behavior, cwd/root and brief propagation, cache/ranking inside `spawn_blocking`, timeout/outcome branches, exact helper warnings, wired module authority, and the ordinary prompt, asynchronous request, and agent-turn call-site prerequisite/application/runtime order. All 351 daemon tests passed serialized in a dedicated target, and final independent review found no unresolved medium-or-higher issue. HTTP adapters, librarian/spec, governance, calls, and broader turn/SSE orchestration remain excluded; existing delegated loader path logs and uncancelled-task/cache-lock amplification remain documented separate risks. Manifest: `docs/specs/2026-07-15-ocean-daemon-longhouse-turn-preparation-extraction-manifest.md`.
