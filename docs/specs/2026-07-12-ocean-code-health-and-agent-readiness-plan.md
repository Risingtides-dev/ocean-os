...
2. Move leaf concerns first:
   ...
   - **Extraction authorized (2026-07-16):** fresh mapping at `5b9e23a` narrowed the first governance move to only `longhouse_demo`, `longhouse_topics`, and `longhouse_topic` in private `src/longhouse_topics.rs`. Characterization `3aedaf1` (rebased from reviewed `c1be830`) freezes empty/full HTTP envelopes, exact 17-event public wire/content/ID/tally relationships, immediate detached acknowledgement, projection-before-publication with the lock scope closed, poison asymmetry, one registry shared by runtime extensions and `AppState`, parent-owned routes/SSE, and whole-owner authority exclusions. Focused groups, all 372 daemon tests, 168 Longhouse tests plus one host-dependent ignore and one doc test, daemon all-target Clippy, formatting/docs/diff checks, and final correctness plus security/lifecycle re-reviews passed with no unresolved medium-or-higher issue on the original baseline. Reconciliation onto `e8f3322` left the three candidate bodies byte-identical and reran the focused characterization/convene/router/SSE/preparation matrix. Real convene remains explicitly deferred to the second security-sensitive governance manifest because it directly couples ready-model filtering and asynchronous council orchestration to durable title grant/bind and raw-token response delivery; no helper trait, substate, or public seam will be invented to split it. Manifest: `docs/specs/2026-07-16-ocean-daemon-longhouse-topic-projection-extraction-manifest.md`.
3. Finish the narrowed topic-projection/demo checkpoint, then separately manifest real convene plus title/control governance before calls and any genuinely remaining state registry/control-plane boundary.
