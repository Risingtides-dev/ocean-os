...
- 3.1 The daemon's room routes authenticate the caller; identity is not a
  ...
  **Proposal** (2026-09-26, PROPOSED, awaiting the operator's yes/no): [`2026-09-26-rooms-member-lane-authentication-proposal.md`](2026-09-26-rooms-member-lane-authentication-proposal.md) resolves the member-lane half. It gives the daemon a verified principal (the operator key or M2 `X-Ocean-Link`, plus `X-Ocean-Acting-Member`), makes leave, close and remove self-or-owner-or-operator, and pins federated rooms to the node human.
  **Evidence** (2026-09-26): before that ruling, `ocean_room_federated_actor_substituted_total{route}` counts federated posts and workspace calls where the caller named someone other than the node human and the daemon acted as the node human anyway (T4). Observability only; nothing is refused. Tests: `t4_*` in `persistent_rooms.rs`, the `a_foreign_human_*` / `the_node_human_an_agent_and_a_refused_caller_count_nothing` trio in `room_workspace_proxy.rs`, `room_metrics_federated_actor_substitution_counts_by_closed_route_label`.
...
- 4.4 The store is durable under load: WAL, busy_timeout, synchronous set in
  ...
  (`wake_channels_are_per_room_and_a_busy_room_cannot_lag_a_quiet_one`).
  **Done** (2026-09-25): store work runs through `off_async_worker`
  (`block_in_place` on the multi-thread runtime) inside both store adapters;
