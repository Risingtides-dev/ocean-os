...
| File | What it holds |
| --- | --- |
| `crates/ocean-daemon/src/extension_registry/mutation/stage_a_gate.rs` | The §20 integrated gate. It uses the real §15 routes, the A3a writer, the supervisor, the lifecycle dispatcher, and ordinary fake-provider turns through `agent_turn`. They run over one real no-op native service. The service speaks the strict stdio protocol, persists its cursor under `data/`, and records every frame it receives. It also has the structural boundary tests. |
...
process tests, not a Rust binary. A Rust fixture binary would need a new
workspace member or build step. The manifest says a new crate requires an
amendment (§6), so A5 did not add one. The script has everything §20 asks of
the fixture: handshake, a cursor persisted under `data/`, pong, ACK, a
...
| 4 enable: identity, roots, minimal env, readiness, one group | same test: `host_hello` identity equals digest, revision, epoch, and floor. `HOME`/`XDG_*`/`TMPDIR`/`PWD`/cwd/argv are checked against the assigned roots by file identity (the host passes descriptor-derived paths, which on macOS are `/.vol/<dev>/<ino>`). The environment names are exactly the §11.3 set, a planted daemon variable is absent, and leader and grandchild share one PGID. | proven |
| 5 new/resumed sessions: scoped metadata, no payloads, client/SSE compatibility | same test: kinds are exactly `session_started, turn_started, turn_finished, turn_started, turn_finished` (the resumed turn has no `session_started`). The prompt and cwd sentinels are absent from every frame. The ACK and ordinary client event types equal a no-extension baseline. | proven |
| 6 lag, then retained replay and reset; turns stay responsive | `G::stage_a_gate_crash_resume_backoff_circuit_and_explicit_retry` (same-epoch resume after a process failure replays exactly the unprocessed facts, with no reset); `G::stage_a_gate_stalled_service_never_delays_ordinary_turns` (20 turns finish while a service never reads); `extension_service::tests::slow_reader_hits_bounded_data_queue_and_records_coalesced_lag` | proven. The child-side `lag` frame content is proven at unit level: `a5_gaps::a5_live_queue_bounds_count_and_bytes_with_computed_replay_availability` |
| 7 disable → events → stale re-enable; project widening | `G::stage_a_gate_local_noop_package_end_to_end` (new epoch, floor ≥ interval high-water mark, `reset: activation_changed`, no interval sequence or session delivered). Widening: `extension_service::tests::activation_epoch_replay_rejects_widening_old_boot_and_ineligible_cursors` (unit). | global: proven. Project-scope widening is unit-level only; see §5 R-2. |
| 8 crash loop: numeric backoff, circuit, generation-safe cleanup | `G::stage_a_gate_crash_resume_backoff_circuit_and_explicit_retry`: measured gaps of 1 s and 2 s between crash starts, `circuit_open` with `restart_count 4`, every leader and grandchild dead, no timer re-close in 3 s, and disable → enable retries with fresh history | proven |
...
| 10 disable one project scope and the global scope | global: steps 7, 11 and 12 of the local gate (filter/epoch, shutdown, child and grandchild reaped, `data/` retained) | global: proven. Project-scope disable: **blocked**, §5 R-2 |
...
| Runtime permission denial fabricates no tool execution | `LC::compatibility_permission_denial_never_fabricates_execution` | partial: adapter level; see §6 O-1 |
...
| A disabled scope observes nothing | `LC::project_scope_is_captured_at_publication_and_delivery_does_not_widen`; live global disable: `G::stage_a_gate_local_…` (interval facts never delivered) | global A5; project scope: see §5 R-2 |
...
| Per-service queue: 256 frames and 1 MiB | `SVC5::a5_live_queue_bounds_count_and_bytes_with_computed_replay_availability` (exactly 256 queued; the rest coalesce into one `lag`; a byte-full queue lags the next frame) | A5 |
...
| Never delays a turn, permission, or session | `G::stage_a_gate_stalled_service_never_delays_ordinary_turns`, `LC::concurrent_publishers_…` (publication is synchronous and non-blocking) | A5 |
| Resume needs a matching boot, epoch and eligible cursor; each reset reason exact | `SVC::activation_epoch_replay_rejects_widening_old_boot_and_ineligible_cursors`, `SVC5::a5_replay_plan_names_every_reset_reason_exactly` (`boot_changed`, `activation_changed` for another epoch or below the floor, `retention_exceeded`, `invalid_cursor`, and the exact `oldest/latest_available`); live: `G::stage_a_gate_local_…` (`activation_changed`, `boot_changed`), `G::stage_a_gate_crash_…` (a valid same-epoch cursor replays with no reset) | A5 |
| Disable → events → re-enable, and widening, cannot replay the interval | global: `G::stage_a_gate_local_…`; widening: `SVC::activation_epoch_replay_…` (unit) | global A5; widening unit-level |
...
| Blocked stdin fails at 2 s | `SVC::blocked_stdin_fails_at_the_two_second_connection_deadline`; live: `G::stage_a_gate_stalled_…` | proven |
...
| Grant preview mutates nothing; stale/wrong confirmation fails; exact applies; diffs stable | `TX::a3a_grant_diff_is_stable_and_confirmation_binds_revision_and_notice`, `TX::a3a_install_trust_enable_…`, `MU::committed_202_is_authoritative_and_a_retry_conflicts_instead_of_recommitting` | proven. Binding-only and native-ack-only diffs are covered by the confirmation hash, not by a separate repeat-preview test (§6 O-8). |
...
| R-2 | **§17 project-scope disable of a shared service** (A3b boundary 2). §17 says a project disable that leaves another effective scope must not stop the shared service; the accepted A2b supervisor restarts it under a new epoch. | §20 step 10 (project scope), §19.1 "one project cannot observe a disabled scope" live, §19.2 project-scope widening live | Every scope change is an activation change: new epoch, restart, preserved restart history. |
...
| --- | --- | --- | --- |
| O-1 | `tool_started`/`tool_finished` and the compatibility-denial arm at the real runtime-bridge call site; `details.cancelled` extraction | The fake provider behind `agent_turn` makes no tool calls; only the adapter is tested | A bridge test with a tool-calling fake provider |
...
| O-8 | Repeat-preview stability for binding-only and native-ack-only diffs | Confirmation binds the canonical diff and notice, tested for widening/narrowing only | Two more preview pairs in `TX::a3a_grant_diff…` |
...
- The stalled-service gate proves a property of the current non-blocking publisher. Breaking that property would mean redesigning the dispatcher.
...
Recorded on the A5 head (see the pull request for the exact commit):
...
| `cargo test -p ocean-daemon` | 1092 passed, 0 failed, 4 ignored |
| `cargo test -p ocean-daemon stage_a_gate` | 7 passed, stable across three consecutive runs |
| `cargo test -p ocean-daemon a5_` | 13 passed, 1 ignored |
| `cargo test -p ocean-daemon a5_stable_reset -- --ignored` | passed (310.5 s) |
...
| `node scripts/check-ledger.mjs` | pass (654 entries) |
...
`cargo +1.88.0 xtask ci --msrv`, and the Ubuntu matrix. The tests need
