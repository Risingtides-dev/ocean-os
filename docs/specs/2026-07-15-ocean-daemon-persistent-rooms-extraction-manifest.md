...
**Date:** 2026-07-15
**Status:** Published through PR #293 merge `92e03bf`; characterization, extraction, dedicated-target validation, compatibility/MSRV/local CI, independent review, and hosted CI passed
**Owner:** Ocean OS
**Rollback point:** rebased characterization commit `6ea6767`
...
This checkpoint started from fetched `origin/main` at merge `3e051c1`, which contains recall-registry PR #287 plus the concurrent three-state permission/TUI work from `827b65b`. Before the extraction commit it rebased onto `cf907a3` (PR #288), whose changes were limited to OMP program documentation/roadmap/events. Before the completion checkpoint it rebased onto `8e5f204` (PR #289), which moved/bounded advisor execution and added `AppState::advisor_limiter` plus matching test-fixture fields but did not change persistent-room, store, call-persistence, LiveKit, route, permission, cwd, or session behavior. Before publication it rebased again onto `39f6f27`, which includes PR #290's advisor-input Clippy fix and PR #291's bounded Longhouse inspection route. The final rebase changed advisor and Longhouse composition plus route-contract fixtures, but no persistent-room/store/call-persistence/LiveKit/permission/cwd/session seam; the room-focused, full serialized daemon, store, workspace, feature, compatibility, MSRV, and canonical gates were rerun. The room auto-convene path continues to use `effective_permission_mode()` and passes `PermissionMode` into `build_prompt_control`; this checkpoint does not restore the older boolean-only policy.
...
Rebased commit `6ea6767` added the approved daemon-only characterization without moving a production persistent-room body or changing `ocean-store`. The real `room_routes()` lifecycle freezes exact create/list/detail/join/message/leave success values, top-level keys, identity/attribution, serde defaults, key/workspace normalization, and transcript ordering. The error matrix freezes custom JSON errors separately from exact Axum JSON/query rejection text, and the mapper table covers every `RoomStoreError` variant. Resolved, unresolved, and agent-authored mention paths now freeze exact returned-row persistence, extension payload/scope, no-false-footprint behavior, and persisted-author → event → audit → spawn source order. Closed-room coverage recombines bounded pages and proves exact equality with snapshot/events plus frozen metadata and roster. One deliberately poisoned shared handle recovers through both lock adapters. Existing paging, restart, call persistence/read-after-close, LiveKit, route-retirement, and static/dynamic precedence coverage remains green.

All 348 daemon tests passed serialized after final upstream reconciliation in dedicated target `/tmp/ocean-target-persistent-rooms`; all 38 `ocean-store` tests passed. Focused room HTTP, trigger, poison, paging, and route groups, formatting, docs/index validation, and diff checks passed. Two fresh boundary/characterization re-reviews reported PASS with no unresolved medium-or-higher issue. The accepted low residuals are deliberate snapshot brittleness in exact Axum/rusqlite text and the source-order test's required `include_str!` redirection to `persistent_rooms.rs` during the mechanical move.

## Extraction result

Rebased commit `801617e` moved the exact approved boundary into the 886-line private `src/persistent_rooms.rs`. The new module owns `RoomStoreHandle`, both poison-recovering synchronous lock adapters, database-path resolution, the error mapper and room DTOs/handlers, mention parsing and resolution, stable UUID-v5 room-agent identity, prompt/context assembly, spawned room-agent turn helper, and open/closed transcript paging. `main.rs` gained only private module/import wiring and redirected the source-order characterization to its new owner.

`AppState`, startup parent-directory creation/open/migration/logging, `room_routes()`, global router/middleware, call `PersistJob`/retry/backoff/drop accounting, `BusSink`, and LiveKit existence/publish authorization remain in composition and consume the same single store handle. A normalized symbol-by-symbol comparison against characterization commit `6ea6767` found every moved body and attached internal comment mechanically identical after only module imports, rustfmt, required `pub(super)` visibility, the permitted handle-alias doc, and the test `include_str!` redirection. Two fresh extraction/security/concurrency reviews reported PASS with no unresolved medium-or-higher issue.

After final reconciliation with PRs #290 and #291, all 348 daemon tests passed serialized, all 38 `ocean-store` tests passed, and the focused room HTTP, poison, trigger, paging, call persistence, LiveKit, route-retirement, and route-precedence groups passed. `cargo check --workspace --tests`, `livekit-tap`, and `deepgram-stt` passed without warnings. `cargo xtask ci --compatibility`, the Rust 1.88 MSRV manifest, and the canonical `cargo xtask ci` local gate passed; the local gate completed workspace tests, all-target denied-warning Clippy, format, docs/index validation, and dependency policy. An earlier pre-PR-289 local-CI attempt reached dependency policy but found no installed `cargo-deny`; after installing the required tool, the unchanged gate passed. A first post-PR-291 canonical-gate attempt exhausted local disk while linking workspace tests; after removing only this workstream's stale dedicated targets, the unchanged gate passed from fresh dedicated target `/tmp/ocean-target-persistent-rooms-ci`. PR #290's upstream `AdvisorInput` grouping superseded and removed this branch's temporary advisor-lint compatibility commit during rebase. Hosted run `29463198497` then passed the default-parallel macOS and Ubuntu repository gates, pinned Rust 1.88 MSRV lane, and cargo-deny lane; PR #293 merged as `92e03bf`.

The inherited low concurrency residual is unchanged: because the initial room-store guard drops before event publication and the audit reacquires it, a concurrent post may receive a sequence between the triggering row and best-effort audit; an audit write failure also remains ignored before spawn. Per-request persisted-row → event → audit-attempt → spawn order remains exact. Live daemon deployment/supervision is owned by the concurrent operator workstream and is not performed by this checkpoint.
