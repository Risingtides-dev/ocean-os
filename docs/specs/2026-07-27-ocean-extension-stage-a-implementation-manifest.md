...
- *The fixture is a shell script*, as in every earlier slice, not the "minimal
  Rust service" §20 specifies. A fixture binary would need a new workspace
  member, and §6 requires an amendment for a new crate, so this deviation is
  listed below as ruling 5. The gate proves host conformance itself: every
  recorded host→child frame is decoded with the strict SDK decoder into its
  declared v1 type and must re-encode to the identical bytes.
- *Assigned paths are compared by inode.* The host passes descriptor-derived
  paths to the child (`/.vol/<dev>/<ino>` on macOS, `/proc/self/fd/<n>` on
  Linux) that only the child can resolve, so the child records the inode each
  of `HOME`, `XDG_*`, `TMPDIR`, `PWD`, cwd, and argv[0] names, and the gate
  compares them with the assigned roots.
- *Live paths the review asked for.*
  - The permission and tool path runs through `agent_turn` on the keyless
...
2. §17 project-scope disable of a shared service (A3b boundary 2). This blocks
   §20 step 10 for a project scope. Project-scope widening is proven live and
   is not blocked.
