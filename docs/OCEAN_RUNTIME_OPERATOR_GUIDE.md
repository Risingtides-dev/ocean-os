...
POST   /v1/rooms/persistent/{key}/artifacts               record what the room produced { id, kind: task|decision|note, title, body?, author_id }; 201 { artifact }. Author must be on the roster (403). 400 invalid_request on a blank id or a blank (whitespace-only) title — refused in the STORE, so the daemon's own callers are held to it too, not only clients. Every create writes a System transcript line in the SAME transaction, so an artifact can never exist that the room's history does not explain.
...
# mkdir, flush, hydrate, and port exposure.
#
