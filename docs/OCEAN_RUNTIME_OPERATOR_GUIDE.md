...
# Extensions (Stage A §15). Reads are credential-free, never execute package code, and carry the cached runtime summary; a 503 extension_state_busy is retryable. Every mutation needs X-Ocean-Operator (503 when the key or header is absent, 403 for invalid/cookie/foreign-origin authority), names expected_state_revision, and answers the common envelope: pre-commit {ok:false, mutation:{operation_id, committed:false, state_revision}, error:{code, message}}; committed {ok:true, mutation:{operation_id, committed:true, state_revision, id, digest, effective, reconciliation: complete|pending|blocked, reap: not_required|complete|pending}} at 200 when reconciliation is complete and nothing is left to reap, else 202 (the commit stands — reinspect, never retry); 500 committed registry_recovery_required when roll-forward failed (restart to recover); 500 outcome_unknown with committed:null if the mutation task died (reinspect by revision before anything else). Bodies must be Content-Type: application/json (415 unsupported_media_type), checked after the credential
GET    /v1/extensions                     every extension with registry state: {state_revision, extensions:[{extension, runtime}]} (?project_id=<registered-uuid>)
POST   /v1/extensions/install             {expected_state_revision, source:{kind:"local-path", path:"/absolute"} | {kind:"git", url:"https://<public-host>[:443]/<path>[.git]", revision:"<exact 40- or 64-hex commit id>"}} — quarantined acquisition outside the state lock, then journaled publication; grants and starts nothing. A Git source is fetched by host git ≥ 2.37 (fixed paths only) with a stripped environment, one public-only DNS resolution, and one http.curloptResolve-pinned connection per attempt (no redirects, credentials, proxies, submodules, LFS/filters, symlinks, or checkout), under a 60 s deadline and a 512 MiB scratch ceiling. Git refusals: 400 invalid_git_source | git_host_not_public | git_revision_mismatch | git_tree_unsupported | git_acquisition_limit, 502 git_resolution_failed | git_fetch_failed, 504 git_acquisition_timeout, 501 git_connection_pinning_unavailable (no pinnable git here; there is no unpinned fallback), 500 git_process_cleanup_failed
DELETE /v1/extensions/{id}                {expected_state_revision, purge_state:false} — only when every scope is disabled and the service is reaped (else 409 extension_active; 409 reconciliation_in_progress with retryable:true while a supervisor pass is in flight); revokes install, trust, acknowledgement, bindings, enablement
GET    /v1/extensions/{id}/inspect        installed/trusted/enabled projection plus cached runtime summary (?project_id=<registered-uuid>)
GET    /v1/extensions/{id}/doctor         static state/digest/manifest/trust diagnostics plus cached runtime summary (?project_id=<registered-uuid>)
GET    /v1/extensions/{id}/status         supervisor's cached runtime projection only {id, probe_run:false, services:[…]}; starts and probes nothing
POST   /v1/extensions/{id}/trust          {expected_state_revision, digest, capabilities, service_grants:[{service_id, native_process_ack:true, secret_bindings}], confirm_grant_diff:null|"sha256:…"} — without confirmation a 200 preview {applied:false, committed:false, preview:{added, removed, native_authority_notice, confirmation}}; with the exact confirmation the committed envelope
POST   /v1/extensions/{id}/enable         {expected_state_revision, scope:{kind:"global"}|{kind:"project", project_id}} — requires exact-digest trust and complete service grants
POST   /v1/extensions/{id}/disable        same scope shape; commits the filter removal, then 200 only after any no-longer-effective service is stopped and reaped (bounded reap failure is the committed 202 and keeps update/remove refused)
POST   /v1/extensions/{id}/update         {expected_state_revision, source} (local-path or the same pinned Git source) — package must be disabled and reaped; the new digest is untrusted until a separate trust
...
GET    /v1/rooms/persistent/{key}/inspect                 Rooms Phase 2 Stage 2a operator projection, read-only and credential-free: room record (id/name/timestamps/closed/workspace_root/trigger_policy), participants, `access`, `federated` (a bool from RoomCredential presence — the bearer is never serialized), `owner` {member_id, eligible}, `execution` {node:"local", cwd, cwd_source: room_workspace_root | unbound — an authorized turn without a live canonical workspace is REFUSED with workspace_unavailable, never run from the daemon cwd}, and `agents[]`: every Phase 1 binding projection plus its deterministic (room, agent, generation) `session_id`, `session_exists`, and `execution_node`. `profile`, `credential_slots`, and safe `resources` projections are populated by Stages 2b/2c. Each agent reports its effective execution cwd source (resource_grant | room_workspace_root | unbound); resource-grant paths are never projected. Serves a soft-closed room with closed:true like /snapshot; unknown room is 404 room_not_found
GET    /v1/rooms/persistent/{key}/profile                 Rooms Phase 2 Stage 2b, credential-free: the room's declarative profile (`profile`: repos, tools, credential_slots, default_resource_id, agent_defaults, revision, updated_by/at; null when never written) plus `credential_slots[]` — each slot's STATUS on this node (`resolved` | `missing` | `expired` | `resolver_not_open`, with the resolver that satisfied it) computed by walking its `oauth:`/`env:`/`keychain:` resolvers; a value is never read into the projection. Unknown room is 404 room_not_found
PUT    /v1/rooms/persistent/{key}/profile                 operator-authenticated (`X-Ocean-Operator`; 503 when the operator key or header is absent, 403 for invalid/ambient/foreign authority), replay-safe under `decision_id` (identical content → 200 changed:false, different content or an id the agent-binding ledger consumed → 409 decision_replay_mismatch). Body {decision_id, repos[{alias, remote, default_branch?}], tools[{kind: mcp|plugin|builtin, name, allowed[]}], credential_slots[{name, purpose?, required?, resolvers[]}]}. Validation is total and typed (invalid_repo_remote — a URL or scp target, never a path; duplicate_repo_alias; invalid_tool_kind; duplicate_tool; invalid_credential_slot_name — `[A-Z][A-Z0-9_]*`; invalid_resolver; duplicate_credential_slot; profile_too_large) and writes nothing on refusal. Until Stage 2c any resource_id / default_resource_id / agent_defaults is 400 phase_not_open. 201 on first write, else 200; one content-minimal System audit row (counts and revision only) per real write. A REQUIRED slot that does not resolve refuses every later room-agent admission with 409 credential_slot_missing and an audit naming the slot
GET    /v1/rooms/persistent/{key}/resources               Rooms Phase 2 Stage 2c, credential-free: every contributed-folder grant in the room as its SAFE projection — resource_id (opaque, daemon-minted), display_name, resource_kind (folder), access_mode (list|read|write|execute ladder), authorized_agent_member_ids, status (EFFECTIVE: an expired grant reads `revoked`), generation, expires_at, granted_by/at, owner_node local. `local_root` is never in any response. Unknown room is 404
POST   /v1/rooms/persistent/{key}/resources               operator-authenticated, replay-safe under `decision_id` (room-wide namespace shared with agent-binding and profile decisions). Body {decision_id, display_name, local_root, access_mode, authorized_agent_member_ids[], expires_at?}. The root is validated LAST and canonicalized ONCE: 400 invalid_local_root (not absolute / dot components), local_root_not_found, local_root_not_directory (cannot open an O_DIRECTORY|O_NOFOLLOW handle), dangerous_root (the filesystem root, $HOME itself, or a root that canonicalizes elsewhere through a symlink); 409 root_already_granted for a root with a live grant (an EXPIRED live grant is retired in the same transaction and the new grant proceeds). 201 created / 200 exact replay; one content-minimal System audit row (alias, id, mode, generation — never the root)
GET    /v1/rooms/persistent/{key}/resources/{resource_id} one grant's safe projection; 404 resource_not_found
DELETE /v1/rooms/persistent/{key}/resources/{resource_id} operator-authenticated revoke (body {decision_id}); terminal, bumps generation, audit `room.resource.revoked`; anything out of revoked is 409 resource_status_conflict
POST   /v1/rooms/persistent/{key}/resources/{resource_id}/suspend  operator-authenticated (body {decision_id}); available -> suspended, bumps generation; a no-op transition consumes the decision without bumping
POST   /v1/rooms/persistent/{key}/resources/{resource_id}/resume   operator-authenticated (body {decision_id}); suspended -> available, bumps generation. A room-agent turn's cwd follows manifest §5: the agent's `agent_defaults` grant, then the profile `default_resource_id` grant (each must be available, authorize the agent, and still canonicalize to a directory), then `Room.workspace_root`, else the turn is refused workspace_unavailable — `inspect` reports the rule per agent as `agents[].execution.cwd_source` (resource_grant | room_workspace_root | unbound) and never projects a grant's path
POST   /v1/rooms/persistent/{key}/resources/{resource_id}/list    Rooms Phase 2 Stage 2d operator PREVIEW of the admitted `room_list` tool: operator-authenticated (503/403 as every mutation), body {agent_member_id, path?}. Runs the SAME tool the named agent gets in a turn, under that agent's CURRENT active binding generation (409 agent_binding_required otherwise), through the same daemon authority that re-validates the binding and the grant on every call — so the answer is exactly what the agent would see: non-recursive, at most 2,000 entries, names/kinds/sizes only, symlinks reported as links and never followed. Typed refusals ride in the body (`ok:false, error`: resource_not_found | resource_not_available | agent_not_authorized_for_resource | access_mode_not_granted | stale_generation | invalid_relative_path | path_not_found | path_escapes_root | not_a_directory | deadline_exceeded). Every call, refused or not, appends one `room_resource_audit` row with actor `operator_preview` and a path DIGEST, never the path. The response carries `via`, `agent_member_id`, `binding_generation`; never `local_root`
POST   /v1/rooms/persistent/{key}/resources/{resource_id}/read    Rooms Phase 2 Stage 2d operator PREVIEW of the admitted `room_read` tool: body {agent_member_id, path, offset?, max_bytes?}. Same gate and authority as `/list`; returns one UTF-8 chunk (default 64 KiB, max 512 KiB, never split inside a code point) with `next_offset`, `file_size`; files over 8 MiB are `file_too_large`, files with NUL bytes are `binary_not_supported`, plus `not_a_file` and `offset_out_of_range`. In a real turn the agent calls `room_list {resource_id, path?}` / `room_read {resource_id, path, offset?, max_bytes?}` directly; both are reserved tool names an ambient provider can never impersonate, appended after the Phase 1 capability intersection, and present only when at least one grant admits the agent
...
The same body also carries the `ocean_room_*` room and federation families listed under "Reading the bridge" in `docs/OPERATIONS.md`. One of them, `ocean_room_federated_actor_substituted_total{route="post"|"workspace_read"|"workspace_command"}`, counts federated-room writes where the caller named a member other than this node's human and the daemon acted as the node human anyway (the room's single Bedrock identity). It refuses nothing; it is evidence for the member-lane enforcement decision. Each increment also logs one `warn` line with the room key and route, never the member ids. `/health` `rooms.federated_actor_substituted` mirrors it by route with a `_total`.
