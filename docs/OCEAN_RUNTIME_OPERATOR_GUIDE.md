...
POST   /v1/rooms/persistent/{key}/attachments             attach a context file: raw bytes, metadata in the query (?filename=&content_type=&uploader_id=). The request Content-Type header is ignored; the query value is what gets recorded. 201 { attachment }. 8 MiB cap (413 attachment_too_large), roster-checked uploader (403), 403 forged_attachment_author when a client claims an Agent/System identity. Every upload writes a System transcript line in the SAME transaction, carrying the sanitized filename and a server-computed byte count only — never the declared content type.
...
GET    /v1/rooms/persistent/{key}/attachments/{attachment_id}  download the bytes. ALWAYS Content-Type: application/octet-stream plus X-Content-Type-Options: nosniff — the declared type is recorded and never echoed, because reflecting an uploader-chosen text/html at a browser origin is stored XSS. 400 malformed id (refused before any filesystem access), 404 unknown_attachment, 500 attachment_bytes_missing when the row and the disk disagree on length or sha256
DELETE /v1/rooms/persistent/{key}/attachments/{attachment_id}  remove the row and the bytes (?actor_id=<participant>); 200 { removed } and the transcript records who removed it. 400 malformed id, 403 forged/roster, 404 unknown_attachment
