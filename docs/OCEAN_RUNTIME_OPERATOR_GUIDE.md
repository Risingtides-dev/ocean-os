...
POST   /v1/extensions/install             {expected_state_revision, source:{kind:"local-path", path:"/absolute"}} — quarantined acquisition outside the state lock, then journaled publication; grants and starts nothing. source kind "git" is slice A4 and is refused 501 git_connection_pinning_unavailable
...
POST   /v1/extensions/{id}/update         {expected_state_revision, source} — package must be disabled and reaped; the new digest is untrusted until a separate trust
