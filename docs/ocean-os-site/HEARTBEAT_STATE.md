...
## Done pages
- `docs/ocean-os-site/index.html` — architecture overview; latest QA pass removed stale “first pass/next slice” language and confirms no broken internal links.
...
- `docs/ocean-os-site/pages/protocol.html` — SDK vs legacy protocol layers, `/v1/agent/turns`, `/v1/agent/events`, event lifecycle, components/browser activity, compatibility map; freshness pass updated line refs and CTA.
...
- `docs/ocean-os-site/pages/desktop.html` — native GPUI/egui desktop architecture, daemon client contract, surface-gpui turn submission, session-scoped SSE, canvas/web host.
- `docs/ocean-os-site/pages/cli-sdk.html` — CLI command surface, one-shot prompt flow, existing CLI capture, SDK identifiers/sessions/requests/events, compatibility map; latest freshness pass updated line refs and CTA.
- `docs/ocean-os-site/pages/providers.html` — provider registry, model catalogue, credential resolution, readiness, daemon model API, protocol transport mapping, live `/ready`/`/v1/models` check.
- `docs/ocean-os-site/pages/sessions.html` — local-first session storage, workspace buckets, atomic JSON saves, strict resume semantics, history caps, session APIs, SDK bridge gap.
...
## Next page order
All originally queued core/client pages are filled. Next recommended small slice: source-citation freshness pass on `providers.html` or real TUI/desktop screenshot capture if runnable.
...
## Latest slice
- Completed source-citation freshness pass on `docs/ocean-os-site/pages/cli-sdk.html`.
- Files changed: `docs/ocean-os-site/pages/cli-sdk.html`, `docs/ocean-os-site/HEARTBEAT_STATE.md`.
...
- Gotcha: SDK line numbers have shifted significantly around `AgentTurnEvent` and backward-compat notes; keep future references explicit.
