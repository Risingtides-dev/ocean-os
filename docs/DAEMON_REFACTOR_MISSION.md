...
| Longhouse claim/revoke/recall/breach/board control | Characterized; extraction authorized | Rollback `0e7a4dc` retains seven parent-owned tests and the exact test-only parent import contract freezing exact methods/extractors/envelopes, claim secrecy/lifecycle, no-auth revoke/threshold-one recall/three-report breach plus post-close 200/zero-strike behavior, persisted titles versus memory-only recall, board fold/publication/poison, title poison, and exact 13-item/import/visibility/source authority. All 500 daemon tests, 168 Longhouse tests plus one ignore and one doc test, Clippy/docs/format/diff, and final correctness plus security/lifecycle reviews passed with no unresolved medium-or-higher issue. Real convene remains excluded |
