...
| Longhouse claim/revoke/recall/breach/board control | Manifested; characterization next | Fresh mapping selected the exact 13-definition private `longhouse_governance_control.rs` boundary over existing `AppState` handles and owner-crate algorithms. A separate comment-only checkpoint now states the actual no-auth-extractor local-route posture, caller-supplied recall UUID/threshold-one behavior, accepted-report breach escalation plus the post-close 200/zero-strike response, and in-memory/poison-divergent board semantics. Characterization must freeze exact HTTP/lifecycle/persistence/poison/non-disclosure/source-authority behavior before extraction |
