...
| Longhouse preparation adapters | Manifesting | Proposed private `src/longhouse_preparation.rs` boundary covers only state-free prepare/inspect/workflow HTTP shells; PR #292 exact-token evidence, extractor/method/envelope/privacy, and blocking/read-only behavior require characterization and review before extraction; librarian fetch/spec remain separate after a disclosed symlink-retarget security disposition |
