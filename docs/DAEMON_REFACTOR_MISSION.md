...
**Updated:** 2026-07-28
**Published Phase 2C implementation:** `f64cda59`
**Current manifest baseline:** `7a51f5bb`
...
| Longhouse claim/revoke/recall/breach/board control | Published | Private `src/longhouse_governance_control.rs` owns the exact 13-definition HTTP adapter boundary from rollback `0e7a4dc`; seven parent-owned tests freeze exact methods/extractors/envelopes, claim secrecy/lifecycle, no-auth revoke/threshold-one recall/three-report breach plus post-close 200/zero-strike behavior, persisted titles versus memory-only recall, board fold/publication/poison, title poison, and import/visibility/source authority. The current-main integration passed exact mechanical comparison, all 592 daemon tests, denied-warning Clippy, formatting/docs/diff, and fresh correctness/security review with no unresolved medium-or-higher issue. Real convene remains excluded |
...
At this checkpoint, `main.rs` is approximately 25.2k lines after persistent-room, Observatory/session-config, extension-lifecycle, and Longhouse characterization deliberately expanded the parent test suite, while the room, Longhouse HTTP-preparation, turn-preparation, topic-projection/demo, and exact 13-definition governance-control boundaries moved into private owners. CORS, metrics, pure event adapters, ordinary turn/session-read workspace policy, model catalog and roles, security-sensitive settings, the home-sandboxed filesystem surface, project-registry adapters, Slack Canvas host fulfillment, component-interaction fulfillment, bounded advisor execution, request/permission control records, recall tally storage, persistent-room lifecycle/orchestration, Longhouse advisory preparation, and Longhouse topic observability now have independent private owners. The current 4,796-line persistent-room module owns durable-room HTTP lifecycle, federation-facing wake helpers, and local execution adapters while `AppState`, startup, router composition, call persistence/retries, and LiveKit authorization remain in `main.rs`. The 349-line Longhouse preparation module owns only state-free prepare/inspect/workflow HTTP adaptation; the 333-line turn-preparation module owns only its gate/presentation/deadline/read-only helper; the 273-line topic owner owns only the scripted producer plus list/detail reads; and the 645-line governance-control owner owns only the five characterized mutating HTTP adapters and their request types/helpers. Routes, shared-registry assembly, real convene, librarian/spec compatibility, governance/title/control state construction, and `ocean-longhouse` algorithms remain outside those owners. Characterization tests deliberately keep checked contracts visible even when they increase raw line count.

## Course from here

1. The [Longhouse governance-control checkpoint](specs/2026-07-19-ocean-daemon-longhouse-governance-control-extraction-manifest.md) is published. Separately manifest real convene/title grant-bind before calls and turn/SSE orchestration.
