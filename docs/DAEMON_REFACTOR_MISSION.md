...
| Longhouse preparation adapters | Extraction authorized | Proposed private `src/longhouse_preparation.rs` boundary covers only state-free prepare/inspect/workflow HTTP shells; exact extractor/method/envelope/privacy, PR #292 evidence, cwd confinement, and blocking/read-only behavior are characterized and freshly reviewed; librarian fetch/spec remain separate after a disclosed symlink-retarget security disposition |
