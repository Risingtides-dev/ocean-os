...
| Longhouse turn preparation/presentation | Extraction authorized | Proposed private `src/longhouse_turn_preparation.rs` boundary covers only the fresh opt-out gate, cached read-only preparation under the existing blocking deadline, and deterministic advisory rendering/application; exact presentation/no-op, environment, cwd/cache/blocking/deadline/fail-open, helper-warning, module-authority, and all three call-site ordering contracts are characterized at `f6e8efe` and independently reviewed |
...
At this checkpoint, `main.rs` is approximately 20.6k lines after persistent-room and Longhouse-preparation characterization deliberately expanded the parent test suite, while the 886-line room and 334-line Longhouse preparation implementation boundaries moved into private owners. CORS, metrics, pure event adapters, ordinary turn/session-read workspace policy, model catalog and roles, security-sensitive settings, the home-sandboxed filesystem surface, project-registry adapters, Slack Canvas host fulfillment, component-interaction fulfillment, bounded advisor execution, request/permission control records, recall tally storage, and persistent-room lifecycle/orchestration now have independent private owners. The 238-line request-control module owns storage mechanics and exact bounded transitions only. The 52-line recall-registry module owns memory-only tally construction, casting, poison recovery, and named removal only. The current 887-line persistent-room module (after a comment-only owner clarification) owns the shared handle/lock adapters, durable-room HTTP lifecycle/paging, and room-agent auto-convene helper path; `AppState`, startup, router composition, call persistence/retries, and LiveKit authorization remain in `main.rs`. The 349-line Longhouse preparation module (including its owner header/imports and required parent visibility) owns only state-free prepare/inspect/workflow HTTP adaptation; routes, turn-time preparation, librarian/spec compatibility, governance state, and `ocean-longhouse` algorithms remain outside it. Characterization tests deliberately keep checked contracts visible even when they increase raw line count.
