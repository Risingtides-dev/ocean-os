...
Routes (all in `crates/ocean-daemon/src/main.rs`; JSON routes use typed `{ ok, error }` bodies with 400 on a bad key and 404 on an unknown room; SSE is streaming; retry returns strict status codes). The table below is the COMPLETE room surface — one row per registered method and path, forty-two of them, in the order `room_routes()` registers them — and it is executable, not descriptive: `room_route_table_and_architecture_route_counts_are_in_parity` in `crates/ocean-daemon/src/main.rs` fails if the router grows a `/v1/rooms` route this table does not carry, or if this table carries one the router does not register.

The **Auth** column names what a caller must present BEYOND reaching the daemon. `Loopback trust only` is the honest reading of most of them: the daemon is a local trust boundary with no HTTP authentication of its own (see `OCEAN_BIND` and the CORS whitelist in `OCEAN_RUNTIME_OPERATOR_GUIDE.md`), so on those routes reaching the socket IS the grant. Every non-GET/HEAD/OPTIONS route, room or not, additionally refuses a request carrying `Cookie` or a present `Origin`/`Referer` outside that CORS trust set (403 `ambient_credential_rejected` / `foreign_origin_rejected`, `crates/ocean-daemon/src/cross_site_write.rs`), so a browser page cannot drive a roster-asserted write DIRECTLY with a no-preflight simple request; an absent `Origin` passes, as it does on the operator lane, which means a write laundered through an auth-off surface proxy is not covered here (ocean-surface #230). Every request on every method also needs a local `Host` (421 `host_not_allowed`, `host_guard.rs`). `Roster-asserted` means the caller NAMES an identity and the daemon checks that name against the live roster inside the same guard as the write — an assertion the room can refuse, not a credential the client holds. The federated and workspace lanes never take a client-supplied bearer at all: the room's Bedrock credential stays in this process.

| Method | Path | Purpose | Auth |
| --- | --- | --- | --- |
| `GET` | `/v1/rooms/persistent` | List this daemon's persistent rooms as ONE BOUNDED PAGE (`?limit=&cursor=`, default 100, capped at 1000), with `next_cursor`, `has_more`, per-room `read_states`, and a sparse same-page `attention` projection beside it — a client past the cap must page rather than read any array as the whole set. | Loopback trust only; attention identity is daemon-derived and never caller-supplied |
...
| `POST` | `/v1/rooms/persistent/{key}/close` | Soft-close one persistent room and append its durable close marker. | Roster-asserted `?actor_id=` or `X-Ocean-Operator`; a presented operator header selects that lane and never falls back |
...
| `POST` | `/v1/rooms/persistent/{key}/participants/{participant_id}/retire` | Retire a placeholder human (`surface-operator`, `web-<16 hex>`) into a live member: moves owner role and agent ownership, records an alias, appends an audit row. | `X-Ocean-Operator`; replay-safe decision id |
...
| `GET` | `/v1/rooms/persistent/{key}/inspect` | Read Room access, owner, agent bindings, profile references, credential-slot status and safe resource grants, including each agent's effective cwd source without a grant path. | Loopback trust only; credential-free read |
| `GET` | `/v1/rooms/persistent/{key}/profile` | Read references-only Room profile and this node's credential-slot status; never credential values. | Loopback trust only; credential-free read |
| `PUT` | `/v1/rooms/persistent/{key}/profile` | Update Room profile references with a replay-safe decision and content-minimal audit. | `X-Ocean-Operator`; decision id required |
| `GET` | `/v1/rooms/persistent/{key}/resources` | List safe local contributed-folder grant projections without local roots. | Loopback trust only; credential-free read |
| `POST` | `/v1/rooms/persistent/{key}/resources` | Grant a validated local folder with generation-bound agent access. | `X-Ocean-Operator`; replay-safe decision id |
| `GET` | `/v1/rooms/persistent/{key}/resources/{resource_id}` | Read one safe grant projection with effective expiry status. | Loopback trust only; credential-free read |
...
| `POST` | `/v1/rooms/maintenance/run` | Run the bounded retention and attachment-orphan maintenance pass on demand. | `X-Ocean-Operator`; no room-member lane |
...
room outside the requested page. The identity is daemon-owned: Local rooms use
the one local cursor principal and report `mention_count: 0` because local
transcripts carry no structured addressee; federated rooms use only the stored
credential's `local_human_member_id` plus the durable mirrored cursor. Exact
federated addressees are normalized from Bedrock's validated
`mention_member_ids` at confirmed ingest. No query/body identity, fuzzy body
parse, Bedrock request, or per-room tail participates. Global sequence gaps are
counted by durable rows, not subtraction, and an authoritative mirror clear
makes the retained confirmed rows unread again.
...
**Room close** is `POST /v1/rooms/persistent/{key}/close`, the first way a member or an operator can end a persistent room from a shipped surface. `closed_at` and every soft-closed read path have existed for as long as durable rooms have, but production reached them from exactly one place — a call ending — so a room somebody made could be joined, written, summarized and federated and never ended. Two authorities and the header picks which: `?actor_id=` naming a roster member, roster-checked inside the closing transaction and refused with 403 `forged_closer` if it claims an Agent's or System's identity; or an `X-Ocean-Operator` credential, which is deliberately NOT roster-checked (operator authority is over the daemon, not membership in one room) and whose PRESENCE selects that lane outright — a credential that was presented and failed is refused rather than silently downgraded to whatever `actor_id` came with it. One IMMEDIATE transaction refuses a room that is not open with the same flat 404 every other write to a closed room gets, appends a System marker naming the closer, and sets `closed_at`, so a frozen room always says who froze it and when; the marker's prose is filtered like every other caller-supplied string a transcript line quotes. 200 answers `{ room, closed: true, marker_seq }`. Afterwards the room is FROZEN, not gone: room detail 404s, writes refuse, `/snapshot` answers `closed: true` with the transcript pageable in both directions and the roster and `agent_owners` intact, and a live `/events` tail flushes the close marker and then ENDS rather than idling on a room that can never produce another row — which is what the route's `/events` 404 on a closed room already implied, since a client could not have re-established that connection anyway. Closing is local: a federated room's credential, outbox, and access projection are untouched, and nothing is sent to Bedrock.

**Room not open (DoD 1.10)** is one answer on every route: a room that never existed or is soft-closed is a 404 whose body carries `room_not_open: true` and a `code` (`room_not_found`, except the attachment upload's pre-existing `unknown_room`, kept because ocean-surface reads it). Existing `error` values are unchanged, so the marker is additive. The audit reads are the deliberate exception and admit it: `/transcript`, `/snapshot`, and the attachment list answer a soft-closed room 200 with `closed: true`, and a room that never existed on those routes still gets the 404. `every_room_route_gives_one_answer_for_a_room_that_is_not_open` pins it across room detail, events, summarize, messages, and the three audit reads.

**Room maintenance** is `POST /v1/rooms/maintenance/run` plus a 6-hour daemon loop, and the `room_maintenance` object on `GET /health` is how an operator sees either. Two sweeps share the loop. Transcript retention cuts rooms CLOSED longer than `OCEAN_ROOM_RETENTION_DAYS` — transcript, attachment rows and bytes, read cursors, and the room's `federated_events` dedup index, one transaction per room — and never an open room at any age, because the window is measured from the close. The `rooms` row survives a cut, so a cut room still answers `closed: true` and an empty transcript rather than 404ing as one that never existed; the dedup index does NOT survive it, because `ingest_confirmed_event` cross-checks an index tuple against the transcript row it names and a dangling one would stop the room ingesting forever (the ordering baseline still holds through the retained `room_access` cursor). Attachment orphan GC unlinks blob bytes no `room_attachments` row claims, and whole directories matching no room the store knows — the blob path is a one-way hash of the room key, so the sweep derives the expected directory of every room including closed ones and treats what is left as nobody's. Anything modified within the last hour is spared: the upload path fsyncs bytes BEFORE committing the row, so every successful upload spends a moment unreferenced. `OCEAN_ROOM_RETENTION_DAYS` is read once at startup; unset, `0`, and anything that does not parse are all NEVER, which is the default, because a transcript cut is unrecoverable and a mis-parse must never become a shorter window than the operator wrote. The route is operator-authenticated with no member lane (the sweep is store-wide and belongs to no room) and answers with the same card. The card carries the configuration — `interval_secs`, `retention_days`, `orphan_grace_secs` — beside the counts, because `rooms_cut: 0` alone cannot distinguish a clean sweep from retention that was never turned on. Additive and absent from pre-field daemons.
