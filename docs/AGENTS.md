...
- `specs/2026-07-14-ocean-extensions-architecture-and-migration-manifest.md` governs the approved extension architecture and staged migration. Phases 0–1 are accepted; Phase 1 owns strict installed/trusted/enabled state plus static no-execution inspect/doctor reads. `specs/2026-07-27-ocean-extension-stage-a-implementation-manifest.md` is the operator-ratified 2026-07-27 exact Crew Stage A / Extension Phases 2–3 contract; it authorizes only the ordered A1 → A2a → A2b → A3a → A3b → A4 → A5 implementation sequence and its binding review/stop gates. The corrective A1 candidate is implemented as strict additive SDK wire DTOs and an unwired pure daemon lifecycle adapter with bounded correlation, lifecycle-owned request-scoped atomic terminal authorities, cleanup, and boot-ring retention, but remains at its independent acceptance gate; A2a is blocked on that PASS. A1 supplies no service process, transport, registry mutation, route, or live producer.
