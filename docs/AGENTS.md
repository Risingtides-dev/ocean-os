...
- `specs/2026-07-14-ocean-extensions-architecture-and-migration-manifest.md` governs the approved extension architecture and staged migration. Phases 0–1 are accepted; Phase 1 owns strict installed/trusted/enabled state plus static no-execution inspect/doctor reads. `specs/2026-07-27-ocean-extension-stage-a-implementation-manifest.md` is the operator-ratified exact Stage A contract and authorizes only A1 → A2a → A2b → A3a → A3b → A4 → A5 under its review gates. A1 is accepted. A2a is implemented on its dedicated branch pending independent acceptance: a strict additive service-grant reader in the sole registry authority and minimum exact-grant native hello/ready supervision with descriptor/file-id-bound executable and assigned-root generations on macOS/Linux, minimal environment, explicit `env:` secret consumption, cause-preserving generation-safe bounded Unix cleanup, unsupported-platform cache rows, and non-probing cache reads. A2a has no registry mutation or durable first-Stage-A-publication marker, so it continues to accept only the ratified A0 missing-companion form; A3a must atomically add the marker that makes later companion absence fail closed. Lifecycle wiring/replay, health/restarts, registry mutations/routes/CLI, and every A2b+ concern remain absent.
