...
- `specs/2026-07-14-ocean-extensions-architecture-and-migration-manifest.md` governs the approved extension architecture and staged migration. Phases 0–1 are accepted; Phase 1 owns strict installed/trusted/enabled state plus static no-execution inspect/doctor reads. `specs/2026-07-27-ocean-extension-stage-a-implementation-manifest.md` is the operator-ratified exact Stage A contract and authorizes only A1 → A2a → A2b → A3a → A3b → A4 → A5 under its review gates. A1 and A2a are accepted. This branch implements A2b pending independent review: the nine authoritative metadata-only lifecycle producers, guarded request terminal and host tool correlation, synchronous project snapshots with immutable activation epochs, bounded boot-local replay/live delivery and prioritized controls, concurrent replay ACK draining with fixed ACK state, cancellation-aware aggregate attach, mismatch-safe authoritative project identity, checked project epoch/reap reconciliation, scope-stable circuit history, bounded redacted stderr, post-write-only ACK eligibility, and immediate bounded exceptional cleanup retry with retained process/temp-root authority for reconciliation and shutdown. It adds no registry mutation, route, CLI, durable replay, or A3+ behavior. A2a–A2b have no durable first-Stage-A-publication marker; only A3a may atomically introduce the marker that makes later `service-grants.json` absence fail closed, and A3a remains blocked until A2b acceptance.
