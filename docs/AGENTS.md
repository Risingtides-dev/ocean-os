...
- `specs/2026-08-09-ocean-team-manager-gate1-manifest.md` is a proposal awaiting operator ruling. It defines the candidate-versus-commitment boundary, transcript-authoritative work model, authenticated governance, privacy-safe Team Home, and ordered Gate 0–8 program. It classifies the current dirty `ocean-board`/daemon/TUI work as an unshipped prototype, authorizes no implementation before acceptance, and keeps agent execution blocked on a separately accepted Crew Stage B-or-later implementation manifest.
