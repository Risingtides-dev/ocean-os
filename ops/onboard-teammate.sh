...
#      alone unless --force.
#   5. LaunchAgent dev.risingtides.ocean-daemon — the package daemon is copied to
...
#   6. ocean-mcp setup, and `claude mcp add` when Claude Code is on PATH.
#   7. Prints the remaining manual checklist (provider login, invite redeem,
#      doctor).
#
...
usage() {
  cat <<USAGE
Usage:
  ops/onboard-teammate.sh --model MODEL [--bedrock-url URL] [--force] [--dry-run]
  ...
  --force              Replace a federation.env that carries a real owner
                       credential, and a supervised daemon artifact that was
                       built from a repo checkout.
...
MODEL=""; BEDROCK_URL="$DEFAULT_BEDROCK_URL"; FORCE=0; DRY=0
while [[ $# -gt 0 ]]; do
  case "$1" in
...
# ── 4. federation.env ──────────────────────────────────────────────────────
say "[4/7] federation.env — $ENV_FILE"
...
# ── 5. the supervised daemon ───────────────────────────────────────────────
say "[5/7] LaunchAgent $LABEL (package daemon, neutral cwd \$HOME, OCEAN_YOLO=1, OCEAN_MODEL=$MODEL)"
package_version() {
  local pkg_json="$1"
...
# ── 6. ocean-mcp ───────────────────────────────────────────────────────────
say "[6/7] ocean-mcp — the Ocean skill and MCP server for Claude Code / Codex"
...
say "[7/7] done. Next, in this order:"
cat <<CHECKLIST
    ...
    3. Verify:                                        ocean-mcp doctor
       and in Claude Code ask it to read the room (the ocean_room_read tool).
