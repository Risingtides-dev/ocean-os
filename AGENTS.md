...
- The active extension architecture and staged migration program is governed by `docs/specs/2026-07-14-ocean-extensions-architecture-and-migration-manifest.md`; Phases 0–1 are accepted. Phase 1 now includes strict installed/trusted/enabled state plus static inspect/doctor reads; install/enable mutations, lifecycle observers, and service supervision remain pending in Phases 2–3. The operator-accepted Phase 6 orchestration-transport ratification is `docs/specs/2026-07-18-ocean-crew-orchestration-and-durable-workflow-manifest.md`; the exact Stage A implementation contract at `docs/specs/2026-07-27-ocean-extension-stage-a-implementation-manifest.md` was operator-ratified on 2026-07-27 and authorizes only its ordered A1 → A2a → A2b → A3a → A3b → A4 → A5 sequence. The corrective A1 candidate is implemented as additive SDK v1 DTOs plus an unwired pure daemon lifecycle adapter with bounded correlation, lifecycle-owned request-scoped atomic terminal authorities, cleanup, and boot-ring retention; it launches nothing and changes no live producer. A1 remains at its independent acceptance gate, so A2a is authorized by sequence but blocked until that review passes. Crew Stages B–E each require their own implementation manifest. It places extension-provided Longhouse Undertow/Offshore facade tools and intersected role capability profiles over an extension-owned Crew engine, not in core.
