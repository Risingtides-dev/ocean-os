...
Use `crates/AGENTS.md` for all 28 current workspace packages, ownership exclusions, entry points, local contracts, and narrow validation commands.

## Work Guidance

- Current architecture and operations live in `docs/ARCHITECTURE.md` and `docs/OPERATIONS.md`; open work lives in `ROADMAP.md`.
- The active extension architecture and staged migration program is governed by `docs/specs/2026-07-14-ocean-extensions-architecture-and-migration-manifest.md`; Phase 0 is accepted, and the Phase 1 schema/tool-lane checkpoint is implemented but not accepted. Installed/trusted/enabled state and inspect/doctor remain pending. The Phase 6 orchestration-transport ratification is drafted as `docs/specs/2026-07-18-ocean-crew-orchestration-and-durable-workflow-manifest.md` (proposed, not yet accepted; authorizes no code); it places extension-provided Longhouse local/offshore facade tools and intersected role capability profiles over an extension-owned Crew engine, not in core.
- The Ocean Observatory program is governed by `docs/specs/2026-07-17-ocean-observatory-architecture.md` with Gate 0 accepted in `docs/specs/2026-07-17-observatory-gate0-decisions.md` (operator ruling: 90s pixel-game visual parity on truthful real events with a durable event store; security invariants unchanged). The operator accepted the Gate 1 implementation manifest at `docs/specs/2026-07-17-observatory-gate1-implementation-manifest.md` on 2026-07-17; tasks 2–9 are authorized under its strict dependency order and stop conditions. Do not build from the Claude Design export code or the raw global agent firehose.
