...
- The Ocean Rooms distributed-workspace architecture is operator-approved in `docs/specs/2026-08-16-ocean-rooms-distributed-workspace-architecture.md`: Rooms becomes a room-scoped authorization and work context spanning member-contributed folders and compute across Ocean nodes, with Tailscale-backed direct operations, node-local enforcement, and extension-owned orchestration policy. The architecture is ratified; each implementation phase still requires its own accepted manifest.
