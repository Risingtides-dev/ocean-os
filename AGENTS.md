...
- The active extension architecture and staged migration program is governed by `docs/specs/2026-07-14-ocean-extensions-architecture-and-migration-manifest.md`; Phases 0–1 are accepted. The operator-accepted Phase 6 orchestration-transport ratification is `docs/specs/2026-07-18-ocean-crew-orchestration-and-durable-workflow-manifest.md`; the exact Stage A implementation contract at `docs/specs/2026-07-27-ocean-extension-stage-a-implementation-manifest.md` authorizes only its ordered A1 → A2a → A2b → A3a → A3b → A4 → A5 sequence. A1, A2a, and A2b are accepted. A2b supplies authoritative metadata-only lifecycle publication, synchronous project snapshots and activation epochs, bounded replay/live delivery with fixed ACK state, checked project reconciliation, health/restart/circuit policy, and retained generation-safe cleanup without registry mutation, routes, CLI, or A3+ behavior. A3a (the internal transactional registry writer, strict four-file journal/publication/recovery, exact retention, grant preview/apply, expected-revision concurrency, crash fixtures, and the durable first-Stage-A-publication marker that makes later `service-grants.json` absence fail closed) passed independent review and merged to `main` in PR #501; it adds no route, live reconciliation, or Git acquisition. A3b (the §15 HTTP/CLI mutation surfaces behind the local operator principal, the common pre-commit/committed envelopes, startup recovery before readers and services, and revision-serialized supervisor reconciliation/reap) merged to `main` in PR #502; its status note in the manifest records the open credential-class ruling. A4 (pinned public Git acquisition: strict HTTPS grammar, one public-only DNS resolution, one `http.curloptResolve`-pinned stripped-environment `git` process group per attempt, and checkout-free, filter-free tree extraction into the unchanged A3a quarantine/seal) is independently reviewed and merged (PR #503, follow-ups PR #504). A5 (the integrated Stage A gate: the §20 end-to-end gate over the real routes, supervisor, dispatcher, and ordinary turns, plus §19 gap closures, all tests-only; row-by-row evidence in `docs/specs/2026-09-26-ocean-extension-stage-a5-acceptance-evidence.md`) merged to `main` in PR #505 (live stalled-write bound widened in #506); its §6 open engineering items O-1–O-10 are closed by tests-only follow-up work on `test/extension-stage-a-open-items`, which also records O-11 (the 2 s blocked-stdin deadline is read per frame, bounded by the ACK window and heartbeat). Stage A is not accepted. Acceptance still needs five operator rulings: the mutation credential class, §17 project-scope disable of a shared service, Windows R5, local-source path collisions, and whether the shell §20 fixture stands in for the specified Rust service. It also needs the operator's separate acceptance. Crew Stages B–E each require their own implementation manifest and keep extension-provided Longhouse facades over an extension-owned Crew engine, not core.
- The Ocean Rooms distributed-workspace architecture is operator-approved in `docs/specs/2026-08-16-ocean-rooms-distributed-workspace-architecture.md`, with accepted Gate 0 security and scope rulings in `docs/specs/2026-08-17-ocean-rooms-gate0-decisions-and-threat-model.md`: Rooms becomes a room-scoped authorization and work context spanning member-contributed folders and compute across Ocean nodes, with Tailscale-backed direct operations, node-local enforcement, and extension-owned orchestration policy. `docs/specs/2026-08-25-ocean-rooms-phase1-room-agent-authorization-manifest.md` is the accepted Phase 1 local room-agent binding contract; its implementation landed through Ocean OS PR #425 and Ocean Surface PR #177 on 2026-08-31 after exact-candidate review and green local/GitHub gates. Its rollout invariants and §15 closures remain binding. Stage 2 contributed folders is not opened by that landing, and it plus every later phase still require their own accepted manifests. `docs/specs/2026-09-08-ocean-rooms-phase2-room-profile-and-contributed-folders-manifest.md` is the operator-ACCEPTED (2026-09-08) Phase 2 contract (room profile of references, never tokens; local folder grants; credential-slot status resolution) with its four §11 rulings binding. All four stages are landed and proven live (2a inspect #454, 2b profile + credential-slot status #455, 2c folder grants + the §5 cwd rule #456, 2d admitted `room_list`/`room_read` tools + per-call authority + audit + operator preview #457); the ROADMAP goal closed 2026-09-08. Phase 3 (node identity, remote read-only resources) needs its own accepted manifest before any networking work.
